...
function buildPiiRegistry_(options) {
  ...
  if (opts.contextText) {
    ...
    contextNames.forEach(function(n) { names.push(n); });
  }
  ...
  var sorted = Object.keys(deduped).sort(function(a, b) { return b.length - a.length; });
...
function extractNamesFromContext_(contextText) {
  ...
  var userNameMatch = contextText.match(/^\s*-\s*利用者名:\s*([^\(\n]+?)\s*\(([^\)\n]+)\)/m);
  if (userNameMatch) {
  ...
  var jsonNameKeys = ['Name__c', 'NameKana__c', 'NameFurigana__c', 'Name', 'NameKana', 'NameFurigana', '氏名'];
...
function nextToken_(idx) {
  ...
  if (n > 999) {
    ...
    throw new Error('[PiiMasker] エンティティ数が上限 999 を超えました。コンテキスト分割または上限引き上げを検討してください。');
  }
...
function test_PiiMasker_roundtrip() {
  ...
  var input = '本日 山田 太郎 様の面談を タナカ ハナコ が担当。立ち会い: 佐藤 一郎。';
  ...
  if (restored === input) {
    ...
    Logger.log('❌ roundtrip FAILED (input.length=' + input.length + ', restored.length=' + restored.length + ')');
  }
}
...
function test_PiiMasker_orderingLongestFirst() {
  ...
  if (restored === input) {
    ...
    Logger.log('❌ ordering FAILED (input.length=' + input.length + ', restored.length=' + restored.length + ')');
  }
}
...
function test_PiiMasker_extractFromContext() {
  ...
  // 「名称不明」は除外
  var expected = ['山田 太郎', 'ヤマダ タロウ', '鈴木 花子', 'スズキ ハナコ', '山田 父', 'ヤマダ チチ', 'タナカ ハナコ'];
  var ok = expected.every(function(e) { return names.indexOf(e) >= 0 }) && names.indexOf('名称不明') < 0;
...
function test_PiiMasker_detectUnknownTokens() {
  ...
  var responseFromLlm = '結論: {{ENTITY_001}} 様、補足: {{ENTITY_999}} という未登録トークン。';
