...
function syncStaffFoldersToSalesforce() {
  ...
  const soql =
    "SELECT Id, GoogleFolderName__c, ServiceType__c FROM StaffStatus__c WHERE GoogleURL__c = null AND GoogleFolderName__c != null AND ServiceType__c != null LIMIT 90";
  ...
  try {
    ...
    console.log(
      `【職員用】${records.length} 件のレコードを取得しました。処理を開始します...`,
    );
    ...
    records.forEach((record) => {
      ...
      if (!googleFolderName || !serviceType) {
        console.warn(
          `ID: ${record.Id} はフォルダ名またはサービス種別が空のためスキップします。`,
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `ID: ${record.Id} (種別:${serviceType} / 氏名:${googleFolderName}) のURLを更新します。`,
        );

        fetchSF("StaffStatus__c", record.Id, updateField);
      } catch (e) {
        console.error(
          `ID: ${record.Id} の処理中にエラーが発生しました: ${e.message}`,
        );
      }
    });
