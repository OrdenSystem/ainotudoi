...
function fetchSF(OBJECT, ID, FIELD) {
  const url = getProp("instance_url") + "/services/data/v51.0/sobjects/" + OBJECT + "/" + ID;
  ...
  try {
    ...
    if (statusCode === 401) {
      console.warn("アクセストークンの期限切れ(401)を検知。更新して再試行します...");
      ...
      if (newToken) {
        ...
        throw new Error("トークンの更新に失敗しました。OAuth設定を確認してください。");
      }
    }
...
function refreshAccessToken() {
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    console.error("エラー: OAuth設定(client_id, client_secret, refresh_token)が不足しています。");
    return null;
  }
  ...
  try {
    // token_url は 000_OAuth.js のグローバル定数（My Domain）。クロス組織回避のため login.salesforce.com を直書きしない
    const response = UrlFetchApp.fetch(token_url + "token", options);
