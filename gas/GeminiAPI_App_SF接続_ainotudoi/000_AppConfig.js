...
function test_AppConfig_All() {
  var checks = [
    { key: 'WAREKI_SS_ID',         type: 'spreadsheet', required: true, desc: '和暦変換マスタ' },
    { key: 'SEIKYU_TASK_SS_ID',    type: 'spreadsheet', required: true, desc: '請求App' },
    { key: 'RECORDER_SHEET_ID',    type: 'spreadsheet', required: true, desc: 'HopeRecorder POST履歴' },
    { key: 'QUEUE_SS_ID',          type: 'spreadsheet', required: true, desc: 'AIテキスト生成キュー' },
    { key: 'USER_ROOT_FOLDER_ID',  type: 'folder',      required: true, desc: '利用者フォルダルート' },
    { key: 'STAFF_ROOT_FOLDER_ID', type: 'folder',      required: true, desc: '職員フォルダルート' }
  ];
...
function dump_AppConfig_Ids() {
  var keys = ['WAREKI_SS_ID', 'SEIKYU_TASK_SS_ID', 'RECORDER_SHEET_ID', 'QUEUE_SS_ID', 'USER_ROOT_FOLDER_ID', 'STAFF_ROOT_FOLDER_ID'];
  var props = PropertiesService.getScriptProperties();
