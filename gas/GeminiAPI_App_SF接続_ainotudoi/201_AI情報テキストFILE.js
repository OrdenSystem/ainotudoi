...
function generateAIContextFile(targetId, options) {
  ...
  try {
    ...
    const selConsult = isFirstRun
      ...
      : buildDateTimeSelector_("01相談記録", "[利用者在籍ID]", userId, "[更新日時]", start, end);
    const rowsConsult = callAppSheetApi(APP_ID, API_KEY, "01相談記録", selConsult);
    Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）
    ...
    const selCase = isFirstRun
      ...
      : buildDateTimeSelector_("ケース記録", "[利用者在籍ID]", userId, "[更新日時]", start, end);
    ...
    const selForms = isFirstRun
      ...
      : buildDateTimeSelector_("帳票マスタ複製登録", "[利用者在籍ID]", userId, "[更新日時]", start, end);
    const rowsForms = callAppSheetApi(APP_ID, API_KEY, "帳票マスタ複製登録", selForms);
    Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）
    ...
    if (isFirstRun) {
      ...
      if (pIds.length > 0) rowsFormDetails = fetchByOrIdsChunked_(APP_ID, API_KEY, "帳票子レコード複製登録", "[帳票マスタ複製登録ID]", pIds);
    } else {
      const selChildToday = `Filter(帳票子レコード複製登録, AND([更新日時] >= DATETIME("${start}"), [更新日時] < DATETIME("${end}")))`;
      rowsFormDetails = rowsFormDetails.concat(callAppSheetApi(APP_ID, API_KEY, "帳票子レコード複製登録", selChildToday));
      Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）

      const pIdsToday = rowsForms.map(f => f["帳票マスタ複製登録ID"]).filter(Boolean);
      if (pIdsToday.length > 0) rowsFormDetails = rowsFormDetails.concat(fetchByOrIdsChunked_(APP_ID, API_KEY, "帳票子レコード複製登録", "[帳票マスタ複製登録ID]", pIdsToday));
    }
