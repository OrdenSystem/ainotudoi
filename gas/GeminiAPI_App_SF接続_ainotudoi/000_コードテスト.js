...
function queryTest() {
  let query = "SELECT FIELDS(ALL) FROM StaffStatus__c WHERE todayDATE__c = true LIMIT 200 OFFSET 400";
...
function queryTest99(){
  ...
  const query = "SELECT Name, (SELECT Name, Number__c, Period__c, ProgressRecord__c FROM On1__r ORDER BY Number__c ASC) FROM FixedInformation__c WHERE Id='" + Id + "' LIMIT 1";
  const response = JSON.parse(doQuery(query));
  const res = response.records[0];
  console.log("res", res);
