...
function diagnose_listStuckRows() {
  ...
  Logger.log('===== 停止中の行一覧 =====');
  Logger.log('総数: ' + stuck.length + ' 件 (うち Bandwidth/quota 関連: ' + bandwidthCount + ' 件)');
  stuck.forEach(function(s) {
    Logger.log((s.isBandwidth ? '⚠️ ' : '   ')
      + 'Row=' + s.rowNumber
      + ' / LogID=' + s.logId
      + ' / status=' + s.status
      + ' / retry=' + s.retryCount
  ...
  if (bandwidthCount > 0) {
    Logger.log('');
    Logger.log('対応: GCS quota が回復したら diagnose_retryStuckRow("LogID") で個別に再投入できます。');
    Logger.log('     一括リトライは diagnose_retryAllBandwidthErrors() で実行可能です。');
...
function diagnose_inspectRow(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    Logger.log('msg(先頭200文字): ' + String(row[4] || '').substring(0, 200).replace(/[\r\n]/g, ' '));
    ...
    try {
      ...
      Logger.log('size     : ' + fileSize + ' bytes (' + (fileSize / 1024 / 1024).toFixed(1) + ' MB)');
      ...
      if (typeof estimateAudioDurationSeconds_ === 'function') {
        ...
        Logger.log('推定尺   : ' + Math.round(sec) + ' 秒 (' + Math.round(sec / 60) + ' 分)');
      }
    } catch (e) {
...
function diagnose_smallDummyGcsUpload() {
  ...
  var fileName = 'diagnose/connectivity_test_' + Utilities.formatDate(new Date(), 'JST', 'yyyyMMdd_HHmmss') + '.txt';
  ...
  var url = 'https://storage.googleapis.com/upload/storage/v1/b/' + bucket
  ...
  var body = resp.getContentText().substring(0, 300).replace(/[\r\n]/g, ' ');
  ...
  if (code === 200) {
    ...
    Logger.log('   GCP Console > IAM > Quota で詳細確認: https://console.cloud.google.com/iam-admin/quotas');
  } else {
...
function diagnose_retryStuckRow(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    if (!/^エラー/.test(status) && status !== 'FAILED') {
      Logger.log('⚠️ LogID=' + logId + ' の status は "' + status + '" のため、リトライしません。');
      Logger.log('   既に処理が進行中、もしくは正常完了している可能性があります。');
      return;
    }
    var prevRetry = Number(values[i][13]) || 0;
    var prevMsg = String(values[i][4] || '').substring(0, 100);
    sheet.getRange(rowNumber, 4).setValue('受付');
    sheet.getRange(rowNumber, 5).setValue('[manual retry @ '
      + Utilities.formatDate(new Date(), 'JST', 'yyyy-MM-dd HH:mm:ss')
      + ' / N reset from ' + prevRetry + '] prev: ' + prevMsg);
    sheet.getRange(rowNumber, 14).setValue(0); // N列リセット（人為的介入で再カウント開始）
    Logger.log('✅ LogID=' + logId + ' Row=' + rowNumber + ' を「受付」に戻し、RetryCount を ' + prevRetry + ' → 0 にリセットしました。');
    return;
  }
  Logger.log('❌ LogID=' + logId + ' が POST履歴 シートに見つかりません');
}
...
function diagnose_retryAllBandwidthErrors() {
  ...
  for (var i = 1; i < values.length && retried.length < MAX_RETRY_PER_RUN; i++) {
    var row = values[i];
    var status = String(row[3] || '');
    ...
    sheet.getRange(rowNumber, 4).setValue('受付');
    sheet.getRange(rowNumber, 5).setValue('[bulk retry @ '
      + Utilities.formatDate(new Date(), 'JST', 'yyyy-MM-dd HH:mm:ss')
      + ' / N reset from ' + prevRetry + '] prev: ' + prevMsg);
    sheet.getRange(rowNumber, 14).setValue(0); // N列リセット（人為的介入で再カウント開始）
  ...
  Logger.log('リトライ件数: ' + retried.length + (retried.length === MAX_RETRY_PER_RUN ? ' (上限到達)' : ''));
  retried.forEach(function(r) {
    Logger.log('  Row=' + r.rowNumber + ' / LogID=' + r.logId + ' / N reset from ' + r.prevRetry + ' → 0');
  });
  if (retried.length === MAX_RETRY_PER_RUN) {
    Logger.log('⚠️ 上限 ' + MAX_RETRY_PER_RUN + ' 件に到達。残りは次回実行してください。');
  }
}
...
function diagnose_retryGcsUploadOnly(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    try {
      ...
      var startMs = new Date().getTime();
      uploadLargeFileToGcsByDriveApi_(fileId, fileSize, mimeType, GCS_BUCKET_NAME, audioPath, token);
      var durMs = new Date().getTime() - startMs;

      Logger.log('✅ アップロード成功 (' + durMs + ' ms, 平均 '
        + ((fileSize / 1024 / 1024) / (durMs / 1000)).toFixed(2) + ' MB/s)');
    } catch (e) {
...
function diagnose_sendOneRowToAppSheet(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    Logger.log('対象テーブル: ' + tableName + ' (App ID 末尾4: ****' + appId.substring(appId.length - 4) + ')');

    if (status !== '推論完了(3/3)' && status !== 'エラー(4/4)') {
      Logger.log('⚠️ status が "推論完了(3/3)" でも "エラー(4/4)" でもありません。続行しますが想定外の状態です。');
    }

    var transcribedText = String(r[12] || '');
    Logger.log('文字起こしテキスト 長さ: ' + transcribedText.length + ' 文字');
    if (transcribedText.length > 32000) {
      Logger.log('⚠️ 32,000 文字超: AppSheet Database の LongText 上限に近い、または超過の可能性');
    } else if (transcribedText.length > 5000) {
      Logger.log('ℹ️ 5,000 文字超: AppSheet 列が LongText 型である必要があります（Text 型なら必ず失敗）');
    }

    var formattedStartTime = r[9] ? Utilities.formatDate(new Date(r[9]), 'Asia/Tokyo', 'HH:mm:ss') : '';
    var formattedCreateDate = r[10] ? Utilities.formatDate(new Date(r[10]), 'Asia/Tokyo', 'yyyy/MM/dd') : '';
    ...
    Logger.log('文字起こしテキスト先頭100文字: ' + transcribedText.substring(0, 100).replace(/[\r\n]/g, ' '));
    ...
    if (code === 200) {
      ...
      Logger.log('   status を "AppSheet反映済み" に更新、N列を 0 にリセットしました');
    } else {
      Logger.log('❌ AppSheet 送信失敗 HTTP ' + code);
      Logger.log('   status は据え置き（次回 processAppSheetSendWorker でも再試行対象）');
...
function diagnose_listVoiceRecordsReady() {
  ...
  try {
    ...
    while (rs.next()) {
      n++;
      Logger.log('  RowID=' + rs.getString(1)
        + ' / file=' + rs.getString(2)
        + ' / 利用者在籍ID(末尾4)=****' + (rs.getString(3) || '').slice(-4)
        + ' / 文字数=' + rs.getInt(4)
        + ' / 登録=' + rs.getString(5));
    }
    rs.close();
    stmt.close();
    if (n === 0) {
      Logger.log('  ※ 利用者選択済みの行なし。AppSheet 上で 音声記録対応 行に 利用者在籍ID を設定してください。');
    } else {
      Logger.log('使い方: diagnose_simulateEnrichFromVoiceRow("<RowID>", "<staffPrompt>")');
    }
  } finally {
    closeCloudSql_(conn);
  }
...
function diagnose_simulateEnrichFromVoiceRow(rowId, staffPromptText) {
  ...
  staffPromptText = staffPromptText || '面談記録を SOAP 形式（S:主観 / O:客観 / A:評価 / P:計画）で要約してください。';
  ...
  try {
    ...
    var rs = stmt.executeQuery();
    if (!rs.next()) {
      Logger.log('❌ Row ID=' + rowId + ' が CloudSQL "音声記録対応" に見つかりません');
      rs.close();
      stmt.close();
      return;
    }
  ...
  if (!userZaisekiId) {
    Logger.log('❌ 利用者在籍ID が未設定。AppSheet で 利用者選択 してから再実行してください。');
    return;
  }
  if (!transcriptText) {
    Logger.log('❌ 文字起こしテキスト が空。Cleaning ステップを通っていない可能性。');
    return;
  }
  ...
  var userFullName = '';
  try {
    var userRows = callAppSheetApi(APP_ID, API_KEY, 'CustomerStatus__c',
      "Filter(CustomerStatus__c, [Row ID] = '" + userZaisekiId + "')");
    if (!userRows || userRows.length === 0) {
      Logger.log('❌ CustomerStatus__c に 利用者在籍ID=' + userZaisekiId + ' が見つかりません');
      return;
    }
    userFullName = String(userRows[0]['CustomerName__c'] || '').trim();
    Logger.log('利用者氏名（先頭2文字）: ' + (userFullName || '').substring(0, 2) + '***');
  } catch (e) {
...
function run_diagnose_simulateEnrichFromVoiceRow() {
  ...
  var staffPrompt = '面談記録を SOAP 形式（S:主観 / O:客観 / A:評価 / P:計画）で要約してください。';
...
function diagnose_listGhostRowsInPostHistory() {
  ...
  for (var j = 0; j < ghosts.length; j++) {
    var g = ghosts[j];
    Logger.log('  行' + g.rowNumber + ': LogID=' + g.logId
      + ' status=' + g.status
      + ' ts=' + g.timestamp
      + ' log=' + g.log);
  }
  if (ghosts.length === 0) {
    ...
    Logger.log('クリーンアップは diagnose_markGhostRowsAsFailed を実行（status=FAILED に固定し processTranscriptionWorker のループから外す）');
  }
}
...
function diagnose_markGhostRowsAsFailed() {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    if (currentStatus === 'FAILED') { skipped++; continue; } // 既に FAILED

    sheet.getRange(rowNumber, 4).setValue('FAILED');
    var prevLog = String(row[4] || '');
    sheet.getRange(rowNumber, 5).setValue(
      '幽霊行（audioFileUrl 空）として確定エラー化 / 旧log=' + prevLog.substring(0, 100)
    );
    modified++;
    Logger.log('  行' + rowNumber + ' LogID=' + logId + ' を FAILED に変更');
  }

  Logger.log('FAILED 化: ' + modified + ' 件 / 既に FAILED でスキップ: ' + skipped + ' 件');
}
...
function test_HopeRecorderStartTranscription_emptyArgs() {
  Logger.log('===== test: 全引数空で HopeRecorderStartTranscription 呼出 =====');
  var sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName(SHEET_NAME);
...
function test_HopeRecorderStartTranscription_audioUrlEmpty() {
  ...
  Logger.log('戻り値: ' + result);
  Logger.log('呼出後 lastRow: ' + sheet.getLastRow() + ' (前: ' + beforeRowCount + ')');

  if (sheet.getLastRow() === beforeRowCount && /受付エラー/.test(String(result))) {
    Logger.log('OK: 行追加なし、受付エラー戻り値を確認');
  } else {
...
function test_HopeRecorderStartTranscription_validShape() {
  ...
  var dummyDriveUrl = 'https://drive.google.com/file/d/1234567890ABCDEFGHIJKLMNOPQR/view?usp=sharing';
  ...
  if (sheet.getLastRow() === beforeRowCount + 1 && /受付完了/.test(String(result))) {
    Logger.log('OK: 行が 1 つ追加された、受付完了戻り値を確認');
    Logger.log('★ クリーンアップ推奨: 上記 LogID の行を手動で status=FAILED に変更するか物理削除してください');
  } else {
...
function diagnose_listStuckRows_inProgress() {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    if (createdAt) {
      try {
        var t = (createdAt instanceof Date) ? createdAt.getTime() : new Date(createdAt).getTime();
    ...
    stuck.push({
      ...
      logId: row[0],
      jobName: row[2] || '(未投入)',
      msgSnippet: String(row[4] || '').substring(0, 120).replace(/[\r\n]/g, ' '),
      retryCount: Number(row[13]) || 0,
  ...
  for (var j = 0; j < stuck.length; j++) {
    var s = stuck[j];
    Logger.log('  Row=' + s.rowNumber
      + ' / LogID=' + s.logId
      + ' / 経過=' + (s.elapsedMin === null ? '不明' : s.elapsedMin + '分')
      + ' / retry=' + s.retryCount
      + ' / job=' + (s.jobName.length > 60 ? '...' + s.jobName.slice(-60) : s.jobName)
      + ' / msg=' + s.msgSnippet);
  }
  if (stuck.length > 0) {
    Logger.log('');
    Logger.log('対応: run_diagnose_processOneStuckRow の logId を上記から選んで書換 → 実行で 1 行ずつ消化');
...
function diagnose_processOneStuckRow(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    Logger.log('===== diagnose_processOneStuckRow LogID=' + logId + ' Row=' + rowNumber + ' =====');
    Logger.log('現在の status: ' + status);

    if (status !== '実行中(2/3)') {
      Logger.log('⚠️ status が "実行中(2/3)" ではないためスキップ。');
      Logger.log('   既に処理が進行している、もしくは別状態（受付/推論完了/エラー等）です。');
      return;
    }
    if (!jobName) {
    ...
    Logger.log('jobName: ' + (jobName.length > 80 ? '...' + jobName.slice(-80) : jobName));
    ...
    try {
      ...
      if (job.state === 'JOB_STATE_FAILED' || job.state === 'JOB_STATE_CANCELLED') {
        ...
        Logger.log('❌ Vertex Batch ジョブが失敗状態。status を エラー(3/3) に更新: ' + errMsg);
        return;
      }
      if (job.state !== 'JOB_STATE_SUCCEEDED') {
        Logger.log('⚠️ ジョブ未完了 (' + job.state + ')。完了を待ってから再実行してください。');
        return;
      }
      ...
      var rawDraftText = '';
      (list.items || [])
        .filter(function(obj) { return obj.name.endsWith('predictions.jsonl'); })
        .forEach(function(obj) {
          var url = 'https://storage.googleapis.com/storage/v1/b/' + GCS_BUCKET_NAME
            + '/o/' + encodeURIComponent(obj.name) + '?alt=media';
          var content = UrlFetchApp.fetch(url, { headers: { Authorization: 'Bearer ' + token } }).getContentText();
          content.trim().split(/\r?\n/).forEach(function(line) {
            try {
              var t = JSON.parse(line)?.response?.candidates?.[0]?.content?.parts?.[0]?.text;
              if (t) rawDraftText += t + '\n';
            } catch (_) { /* ignore */ }
          });
        });
      ...
      if (!rawDraftText.trim()) {
        ...
        Logger.log('❌ predictions.jsonl から有効なテキストが取得できませんでした');
        return;
      }
      ...
      try {
        ...
        sheet.getRange(rowNumber, 4).setValue('エラー(3/3)');
        sheet.getRange(rowNumber, 5).setValue(String(e.message || '').substring(0, 500));
        Logger.log('❌ cleaning 失敗 (' + durMs + ' ms): ' + e.message);
        return;
      }
      var durMs2 = Date.now() - cleaningStartMs;
      Logger.log('✅ cleaning 成功 (' + durMs2 + ' ms, ' + finalCleanedText.length + ' 文字)');
      // フォールバックマーカーが先頭に付いているかを表示（PII を含む可能性があるため先頭 80 文字のみ）
      Logger.log('応答先頭: ' + finalCleanedText.substring(0, 80).replace(/[\r\n]/g, ' '));

      // 4. シート反映
      sheet.getRange(rowNumber, 13).setValue(finalCleanedText);
      sheet.getRange(rowNumber, 4).setValue('推論完了(3/3)');
      sheet.getRange(rowNumber, 5).setValue('');
      Logger.log('✅ status を 推論完了(3/3) に更新、M列に finalCleanedText を書込しました');
      Logger.log('   次回 processAppSheetSendWorker トリガーで AppSheet に送信されます');
    } catch (e) {
      sheet.getRange(rowNumber, 4).setValue('エラー(3/3)');
      sheet.getRange(rowNumber, 5).setValue(String(e.message || '').substring(0, 500));
...
function diagnose_processOneStuckRow_flashOnly(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    Logger.log('===== diagnose_processOneStuckRow_flashOnly LogID=' + logId + ' Row=' + rowNumber + ' =====');
    if (r[3] !== '実行中(2/3)') {
      Logger.log('現在の status=' + r[3] + '（実行中(2/3) 以外なので何もしない）');
      return;
    }
    ...
    try {
      ...
      var list = JSON.parse(listResp.getContentText());
      var rawDraftText = '';
      (list.items || []).filter(function(o) { return o.name.endsWith('predictions.jsonl'); }).forEach(function(o) {
        var u = 'https://storage.googleapis.com/storage/v1/b/' + GCS_BUCKET_NAME + '/o/' + encodeURIComponent(o.name) + '?alt=media';
        var c = UrlFetchApp.fetch(u, { headers: { Authorization: 'Bearer ' + token } }).getContentText();
        c.trim().split(/\r?\n/).forEach(function(line) {
          try {
            var t = JSON.parse(line) && JSON.parse(line).response
      ...
      var registry = null;
      try {
        var staffList = (typeof getStaffListCachedForCleaning_ === 'function') ? getStaffListCachedForCleaning_() : [];
      ...
      var maskedRaw = registry ? maskText_(rawDraftText, registry) : rawDraftText;
      ...
      var flashPayload = {
        ...
        generationConfig: { temperature: HOPE_RECORDER_ACTIVE.cleaning.temperature }
      };

      var parsed;
      try {
        parsed = callGeminiWithKeyRotation_('gemini-2.5-flash', flashPayload, { apiVersion: 'v1beta', maxAttempts: 2 });
      } catch (e) {
        Logger.log('flash も失敗: ' + e.message);
        sheet.getRange(rowNumber, 4).setValue('エラー(3/3)');
        sheet.getRange(rowNumber, 5).setValue('flash 直行も失敗: ' + String(e.message).substring(0, 200));
        return;
      }

      var maskedResultText = parsed && parsed.candidates && parsed.candidates[0]
      ...
      var finalText = registry ? unmaskText_(maskedResultText, registry) : maskedResultText;
      var withPrefix = '[緊急バイパス: pro スキップ / flash 使用]\n\n' + finalText;
      Logger.log('flash 成功 elapsedMs=' + (Date.now() - flashStart) + ' outputLen=' + withPrefix.length);
      ...
      sheet.getRange(rowNumber, 4).setValue('エラー(3/3)');
      sheet.getRange(rowNumber, 5).setValue('flashOnly 処理失敗: ' + String(e.message).substring(0, 200));
      return;
    }
  }
  Logger.log('LogID=' + logId + ' が POST履歴 シートに見つかりません');
...
function diagnose_skipCleaningForStuckRow(logId) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    Logger.log('===== diagnose_skipCleaningForStuckRow LogID=' + logId + ' Row=' + rowNumber + ' =====');
    if (r[3] !== '実行中(2/3)') {
      Logger.log('現在の status=' + r[3] + '（実行中(2/3) 以外なので何もしない）');
      return;
    }
    ...
    try {
      ...
      (list.items || []).filter(function(o) { return o.name.endsWith('predictions.jsonl'); }).forEach(function(o) {
        ...
        c.trim().split(/\r?\n/).forEach(function(line) {
          try {
            ...
            var t = p && p.response && p.response.candidates && p.response.candidates[0]
              && p.response.candidates[0].content && p.response.candidates[0].content.parts
              && p.response.candidates[0].content.parts[0] && p.response.candidates[0].content.parts[0].text;
            if (t) rawDraftText += t + '\n';
          } catch (_) {}
        });
      });
      ...
      var withPrefix = '[緊急バイパス: cleaning スキップ / Vertex Batch 出力をそのまま使用]\n\n' + rawDraftText.trim();
      sheet.getRange(rowNumber, 13).setValue(withPrefix);
      sheet.getRange(rowNumber, 4).setValue('推論完了(3/3)');
      sheet.getRange(rowNumber, 5).setValue('');
...
function diagnose_skipCleaningForAllStuckRows() {
  ...
  Logger.log('===== 一括 cleaning スキップ 対象: ' + targets.length + ' 件 =====');
  if (targets.length === 0) {
    Logger.log('滞留行なし');
    return;
  }
  ...
  for (var j = 0; j < targets.length; j++) {
    var logId = targets[j];
    Logger.log('--- (' + (j + 1) + '/' + targets.length + ') LogID=' + logId + ' ---');
    try {
      diagnose_skipCleaningForStuckRow(logId);
      success++;
    } catch (e) {
