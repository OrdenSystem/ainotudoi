...
function generateAIContextFile(targetId, options) {
  ...
  if (!userId) {
    ...
    Logger.log(
      "⚠️ 注意: 引数 targetId がないため、テスト用IDを使用します: " + userId,
    );
  } else {
  ...
  try {
    ...
    // -------------------------------------------------------
    Logger.log("⏳ 基本情報取得中...");
    const userRows = callAppSheetApi(
      APP_ID,
      API_KEY,
      "CustomerStatus__c",
      `Filter(CustomerStatus__c, [Row ID] = '${userId}')`,
    );
    Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）

    if (!userRows || userRows.length === 0)
      throw new Error(`利用者不在: ${userId}`);
    ...
    if (!isFirstRun) {
      const existingText =
        readTextFileFromChatPDFFolder_(user, latestFileName) || "";
    ...
    if (cardId) {
      const cardRows = callAppSheetApi(
        APP_ID,
        API_KEY,
        "DisabilityCard__c",
        `Filter(DisabilityCard__c, [Row ID] = '${cardId}')`,
      );
      Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）

      if (cardRows.length > 0) {
        profile += `## 【受給者証情報】(DisabilityCard__c)\n`;
        Object.keys(cardRows[0]).forEach(
          (key) => (profile += `- ${key}: ${safe_(cardRows[0][key])}\n`),
        );
        profile += `\n`;
      }
    }
    ...
    // 関係者
    const supporters = callAppSheetApi(
      APP_ID,
      API_KEY,
      "SupportPersonnel__c",
      `Filter(SupportPersonnel__c, [Customer__c] = '${customerRefId}')`,
    );
    Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）

    const legals = callAppSheetApi(
      APP_ID,
      API_KEY,
      "LegalRepresentative__c",
      `Filter(LegalRepresentative__c, [Customer__c] = '${customerRefId}')`,
    );
    Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）
    ...
    const staffList =
      options.cachedStaffList && options.cachedStaffList.length > 0
    ...
    // 5-1) 01相談記録
    const selConsult = isFirstRun
      ? `Filter(01相談記録, [利用者在籍ID] = '${userId}')`
      : buildDateTimeSelector_(
          "01相談記録",
          "[利用者在籍ID]",
          userId,
          "[更新日時]",
    ...
    const rowsConsult = callAppSheetApi(
      APP_ID,
      API_KEY,
      "01相談記録",
      selConsult,
    ...
    // 5-2) ケース記録
    const selCase = isFirstRun
      ? `Filter(ケース記録, [利用者在籍ID] = '${userId}')`
      : buildDateTimeSelector_(
          "ケース記録",
          "[利用者在籍ID]",
          userId,
    ...
    const rowsCase = callAppSheetApi(APP_ID, API_KEY, "ケース記録", selCase);
    Utilities.sleep(1500); // ★API制限対策（429エラー削減のため500ms→1500msに拡大）
    ...
    // 5-3) 帳票マスタ
    const selForms = isFirstRun
      ? `Filter(帳票マスタ複製登録, [利用者在籍ID] = '${userId}')`
      : buildDateTimeSelector_(
          "帳票マスタ複製登録",
          "[利用者在籍ID]",
          userId,
    ...
    const rowsForms = callAppSheetApi(
      APP_ID,
      API_KEY,
      "帳票マスタ複製登録",
      selForms,
    ...
    updateHistoryMap_(
      ...
      "更新日時",
      (r) => {
        // スプレッドシートURL（または該当する列名）を取得
        const ssUrl = safe_(r["スプシURL"]);

        return (
          `- [帳票マスタ] RowID=${safe_(r["帳票マスタ複製登録ID"])} 更新=${safe_(r["更新日時"])}\n` +
    ...
    let rowsFormDetails = [];
    if (isFirstRun) {
      const pIds = rowsForms
        .map((f) => f["帳票マスタ複製登録ID"])
        .filter(Boolean);
      if (pIds.length > 0)
        rowsFormDetails = fetchByOrIdsChunked_(
          APP_ID,
          API_KEY,
          "帳票子レコード複製登録",
          "[帳票マスタ複製登録ID]",
      ...
      const selChildToday = `Filter(帳票子レコード複製登録, AND([更新日時] >= DATETIME("${start}"), [更新日時] < DATETIME("${end}")))`;
      rowsFormDetails = rowsFormDetails.concat(
        callAppSheetApi(
          APP_ID,
          API_KEY,
          "帳票子レコード複製登録",
      ...
      const pIdsToday = rowsForms
        .map((f) => f["帳票マスタ複製登録ID"])
        .filter(Boolean);
      if (pIdsToday.length > 0)
        rowsFormDetails = rowsFormDetails.concat(
          fetchByOrIdsChunked_(
            APP_ID,
            API_KEY,
            "帳票子レコード複製登録",
            "[帳票マスタ複製登録ID]",
    ...
    const rowsVoice = callAppSheetApi(
      APP_ID,
      API_KEY,
      "音声記録対応",
      selVoice,
...
function handleYearRollover_(userObj, latestFileName) {
  try {
    ...
    if (fileYear < currentYear) {
      ...
      Logger.log(
        `🔄 年越し検知: ${fileYear}年のデータをアーカイブします -> ${archiveName}`,
      );
...
function parseHistoryToMap_(text) {
  ...
  for (const line of lines) {
    ...
    if (recordMatch) {
      commit();
      currentKey = `${recordMatch[1].trim()}__${recordMatch[2].trim()}`;
      currentDate = recordMatch[4].trim();
      buffer.push(line);
...
function getTodayRangeJST_() {
  ...
  return {
    start: `${ymd} 00:00:00`,
    end: `${Utilities.formatDate(next, "JST", "yyyy-MM-dd")} 00:00:00`,
  };
}
function buildDateTimeSelector_(t, i, u, d, s, e) {
  return `Filter(${t}, AND(${d} >= DATETIME("${s}"), ${d} < DATETIME("${e}"), ${i} = '${u}'))`;
}

function fetchByOrIdsChunked_(
  appId,
  apiKey,
  tableName,
  parentIdFieldExpr,
...
function checkFileExists_(userObj, fileName) {
  try {
    return getChatPDFFolder_(userObj).getFilesByName(fileName).hasNext();
  } catch (e) {
    return false;
  }
}
...
function getOrCreateChatPDFFolder_(userObj) {
  ...
  if (!id) {
    throw new Error(
      `【データ不備】GoogleURLが未登録か形式が不正です。URL: ${url}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `【Driveアクセス不可】フォルダ(ID:${id})が見つかりません。削除されたか権限がない可能性があります。URL: ${url}`,
    );
  }
}
...
function wrapBlock_(n, b) {
  return `<<<${n}:START>>>\n${b}\n<<<${n}:END>>>`;
}
function extractBlock_(t, n) {
  const s = t.indexOf(`<<<${n}:START>>>`);
  const e = t.indexOf(`<<<${n}:END>>>`);
