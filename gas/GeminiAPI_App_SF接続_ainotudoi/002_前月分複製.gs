...
function AddRecordCopyYYYYMM(seikyuID, yyyymmCategory, appSSdbURL) {
  try {
    ...
    // 2. 前月分に一致するレコードを抽出
    const recordsToCopy = data.filter(
      (row) => String(row[colIdx_yyyymmCat]) === prevYyyymmCategory,
    );

    if (recordsToCopy.length === 0) {
      return (
        "前月分のデータが見つかりませんでした (" + prevYyyymmCategory + ")"
      );
    }
    ...
    const newRows = recordsToCopy.map((row) => {
      ...
      if (header.indexOf("登録日時") !== -1)
        newRow[header.indexOf("登録日時")] = now;
      if (header.indexOf("更新日時") !== -1)
        newRow[header.indexOf("更新日時")] = now;
