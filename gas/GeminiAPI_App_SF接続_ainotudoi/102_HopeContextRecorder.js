...
function enrichCaseRecord_(parameters) {
  ...
  var timeBudgetMs = (typeof p.timeBudgetMs === 'number' && p.timeBudgetMs > 0)
    ? p.timeBudgetMs : null;

  // 2. バリデーション（必須 3 項目）
  if (!transcriptText) {
    return { success: false, error: 'transcriptText is required', code: 'E_NO_TRANSCRIPT' };
  }
  if (!userFullName) {
    return { success: false, error: 'userFullName is required', code: 'E_NO_USER' };
  }
  if (!staffPromptText) {
    return { success: false, error: 'staffPromptText is required', code: 'E_NO_PROMPT' };
  }

  // 3. ログ（PII 含めない）
  console.info('[enrichCaseRecord] start zaisekiId=' + maskId_(userZaisekiId)
    + ' promptKey=' + staffPromptKey
    + ' staffId=' + maskEmail_(staffId)
  ...
  if (cfg && cfg.version === 'v0_disabled') {
    console.warn('[enrichCaseRecord] HOPE_CTX_RECORDER_ACTIVE が v0_disabled のため処理しません');
    return {
      success: false,
  ...
  var contextText = '';
  try {
    contextText = getContextFileContent_(googleFolderUrl, userFullName, userZaisekiId);
  } catch (e) {
    console.warn('[enrichCaseRecord] コンテキスト読込失敗、空で続行: ' + e.message);
  ...
  if (!fullPrompt) {
    console.warn('[enrichCaseRecord] buildPrompt が空文字を返したため no-op で返却');
    return {
      success: false,
  ...
  try {
    ...
    sendSlackNotification('🚨 enrichCaseRecord 失敗 zaisekiId=' + maskId_(userZaisekiId)
  ...
  console.info('[enrichCaseRecord] success zaisekiId=' + maskId_(userZaisekiId)
...
function getStaffListCached_() {
  ...
  if (!APP_ID || !API_KEY) {
    console.warn('[getStaffListCached_] APPSHEET_APP_ID/APPSHEET_API_KEY 未設定、空リストで fall through');
    return [];
  }
  ...
  try {
    ...
    var slimRows = fullRows.map(function(st) {
      return {
    ...
    try {
      ...
      console.warn('[getStaffListCached_] cache 保存失敗（slim化後も上限超過）: ' + cacheErr.message);
    }
    return slimRows;
  } catch (e) {
    console.warn('[getStaffListCached_] StaffStatus__c 取得失敗、空リストで fall through: ' + e.message);
    return [];
  }
}
 ...
 */
var GEMINI_ENRICH_FALLBACK_PREFIX_ = '[フォールバック: gemini-2.5-flash 使用]\n\n';
...
function extractGeminiText_(parsed) {
  ...
  if (!text) {
    var reason = (parsed && parsed.candidates && parsed.candidates[0] && parsed.candidates[0].finishReason) || '不明';
...
function callGeminiForEnrich_(fullPrompt, cfg, options) {
  ...
  var timeBudgetMs = (typeof options.timeBudgetMs === 'number' && options.timeBudgetMs > 0)
    ? options.timeBudgetMs : GEMINI_ENRICH_TIME_BUDGET_MS_;
  ...
  try {
    var parsedPro = callGeminiWithKeyRotation_(cfg.model, payload, rotationOpts);
    return extractGeminiText_(parsedPro);
  } catch (proErr) {
    // pro 全試行失敗。flash にフォールバック。
    // PII を含まない（モデル名・エラーメッセージのみ）ことを確認した上で warn ログ。
    console.warn('[callGeminiForEnrich_] ' + cfg.model + ' 全試行失敗、'
      + GEMINI_ENRICH_FALLBACK_MODEL_ + ' にフォールバック: '
  ...
  if (elapsedMs > timeBudgetMs) {
    var timeoutMsg = 'time budget exceeded after ' + cfg.model
      + ' attempts (elapsed ' + elapsedMs + 'ms, budget '
      + timeBudgetMs + 'ms), skipping '
  ...
  try {
    var parsedFlash = callGeminiWithKeyRotation_(
      GEMINI_ENRICH_FALLBACK_MODEL_, payload, rotationOpts);
    var flashText = extractGeminiText_(parsedFlash);
    console.warn('[callGeminiForEnrich_] '
      + GEMINI_ENRICH_FALLBACK_MODEL_ + ' フォールバック成功 outputLen='
      + flashText.length + ' totalElapsedMs=' + (Date.now() - startMs));
    return GEMINI_ENRICH_FALLBACK_PREFIX_ + flashText;
  } catch (flashErr) {
    // flash も失敗 → 最終 throw。呼出側 enrichCaseRecord_ で success:false 化される。
    console.error('[callGeminiForEnrich_] '
      + GEMINI_ENRICH_FALLBACK_MODEL_ + ' フォールバックも失敗: '
...
function getContextFileContent_(googleFolderUrl, userFullName, userZaisekiId) {
  ...
  if (cacheKey) {
    try {
      ...
      if (cached) {
        console.info('[getContextFileContent_] cache hit zaisekiId=' + maskId_(userZaisekiId)
          + ' len=' + cached.length);
        return cached;
      }
    } catch (cacheErr) {
      // キャッシュ読込失敗は致命でない。ログのみで素通し。
      console.warn('[getContextFileContent_] cache read 例外: ' + cacheErr.message);
    }
  }

  // 2) URL から folder ID 抽出（HopeAIChat.js#getPDFFolderId_ と同じ正規表現）
  var match = String(googleFolderUrl).match(/id=([a-zA-Z0-9_-]+)/)
  ...
  try {
    ...
    throw new Error('Drive folder 取得失敗 idTail=' + maskId_(folderId)
      + ': ' + e.message);
  }
  ...
  while (files.hasNext()) {
    ...
    if (cleanFname.indexOf(cleanUserName) >= 0 && fname.indexOf('AIコンテキスト') >= 0) {
      fileList.push({ name: fname, content: f.getBlob().getDataAsString('UTF-8') });
    }
  }
  ...
  console.info('[getContextFileContent_] zaisekiId=' + maskId_(userZaisekiId)
    + ' fileCount=' + fileList.length + ' totalLen=' + combinedText.length);
  ...
  if (cacheKey && combinedText.length < 90000) {
    try {
      ...
      console.warn('[getContextFileContent_] cache put 例外: ' + putErr.message);
    }
  }
...
function test_enrichCaseRecord_dryRun() {
  Logger.log('[TEST] *** Gemini API を実際に呼び出します。'
    + 'モデル: ' + HOPE_CTX_RECORDER_ACTIVE.model
...
function test_callGeminiForEnrich_proAndFallback() {
  Logger.log('[TEST] *** callGeminiForEnrich_ を直接呼出します。'
    + 'モデル: ' + HOPE_CTX_RECORDER_ACTIVE.model
    + ' / フォールバック先: gemini-2.5-flash / 課金が発生します。 ***');

  // PII を含まないダミープロンプト
  var dummyPrompt = HOPE_CTX_RECORDER_ACTIVE.systemPrompt
    + '\n\n## 利用者コンテキスト\n（テスト用ダミー）'
 ...
 */
function test_getContextFileContent_(googleFolderUrl, userFullName, userZaisekiId) {
  var ctx = getContextFileContent_(googleFolderUrl, userFullName, userZaisekiId || '');
...
function dump_RequiredPropsForFeature2() {
  ...
  Logger.log('===== ' + (missing === 0 ? 'すべて設定済み' : '未設定: ' + missing + ' 件') + ' =====');
}
...
function enrichCaseRecordForAutomation(
  transcriptText, userFullName, userZaisekiId,
  staffPromptText, staffPromptKey, staffId,
  googleFolderUrl
) {
  ...
  if (missing.length > 0) {
    var skipMsg = '[AI処理スキップ] 以下のパラメータが空のため処理を行いませんでした: '
      + missing.join(', ');
    console.warn('[enrichCaseRecordForAutomation] ' + skipMsg
      + ' zaisekiId=' + maskId_(userZaisekiId)
      + ' staffId=' + maskEmail_(staffId));
    return skipMsg;
  }
  ...
  try {
    ...
    var exMsg = '[AI処理エラー] 例外発生: ' + (e && e.message ? e.message : 'unknown');
  ...
  if (!result || !result.success) {
    ...
    if (code === 'E_GEMINI_TIMEOUT') {
      ...
      errMsg = '[AI処理タイムアウト] Gemini が過負荷で時間内に完了できませんでした。'
        + 'しばらく経ってから再実行してください。(' + code + ')';
    } else {
      errMsg = '[AI処理エラー] ' + code + ': '
