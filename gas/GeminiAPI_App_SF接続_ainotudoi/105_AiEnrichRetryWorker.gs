  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (elapsed > RETRY_RUN_BUDGET_MS) {
        console.warn(
          "[retryWorker] run budget exceeded after " +
            i +
            " rows " +
            "(elapsed " +
            elapsed +
            "ms), remaining will retry next trigger",
        );
        break;
      }
      var row = rows[i];
...
function retryOneRow_(conn, row) {
  ...
  console.info(
    "[retryWorker] retry kRowId=" +
      row.kRowId +
      " zaisekiId=" +
      maskId_(row.zaisekiId) +
      " attempt=" +
      (row.retryCount + 1),
  );
  ...
  try {
    ...
    console.warn(
      "[retryWorker] AppSheet lookup failed kRowId=" +
        row.kRowId +
        ": " +
        e.message,
    );
  ...
  if (!promptText || !userInfo.fullName) {
    console.warn(
      "[retryWorker] missing prompt/userInfo kRowId=" +
        row.kRowId +
        " promptLen=" +
        promptText.length +
        " nameSet=" +
  ...
  try {
    ...
    console.error(
      "[retryWorker] enrichCaseRecord_ exception kRowId=" +
        row.kRowId +
        ": " +
        e.message,
    );
  ...
  if (!result || !result.success) {
    console.warn(
      "[retryWorker] enrich failed kRowId=" +
        row.kRowId +
        " code=" +
  ...
  try {
    ...
    console.error(
      "[retryWorker] update failed kRowId=" + row.kRowId + ": " + e.message,
    );
    incrementRetryCount_(retryKey, row.retryCount);
    return "fail";
  }
  ...
  if (updated) {
    ...
    console.info(
      "[retryWorker] race kRowId=" + row.kRowId + " (already updated)",
    );
    try {
      props.deleteProperty(retryKey);
    } catch (_) {}
...
function lookupPromptText_(promptRowId) {
  ...
  var selector =
    'Filter(AIプロンプト, [Row ID] = "' + promptRowId.replace(/"/g, "") + '")';
...
function lookupUserInfo_(zaisekiId) {
  ...
  var selector =
    'Filter(CustomerStatus__c, [Id] = "' + zaisekiId.replace(/"/g, "") + '")';
  var rows =
    callAppSheetApi(appId, apiKey, "CustomerStatus__c", selector) || [];
