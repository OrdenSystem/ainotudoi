...
function test_SimpleGcsUpload() {
  ...
  const CONTENT = 'これはGASからGCSへの接続テストです。\n' + new Date().toString();
  ...
  try {
    ...
    if (code === 200) {
      ...
      if (code === 403) {
        ...
        console.error("1. GASの「プロジェクト設定」でGCPプロジェクト番号が設定されているか？");
        console.error(`2. GCPのIAM設定で、ユーザー「${email}」に「Storage オブジェクト管理者」権限があるか？`);
        console.error("--------------------------------------------------");
      }
    }
...
    
    console.log("アクセストークンのキャッシュ ('service_account_token') を正常に削除しました。");
    SpreadsheetApp.getUi().alert("キャッシュを削除しました。再度Appsheetから処理を実行してください。");
  
  } catch (e) {
    console.error("キャッシュの削除に失敗しました: " + e.message);
    SpreadsheetApp.getUi().alert("エラー: キャッシュの削除に失敗しました。 " + e.message);
  }
}
