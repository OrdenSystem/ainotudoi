...
function listAndMoveAudioFiles() {
  ...
  try {
    const targetItems = getTargetFoldersFromSheet(ss, CONFIG_SHEET_NAME, URL_HEADER_NAME);
      ...
      try {
        ...
        while (files.hasNext()) {
          ...
          if (file.getMimeType().startsWith('audio/')) {
            ...
            allResultRows.push([
              generateRandomId(8), basicInfoId, targetFolderName, now, file.getName(), fileUrl, '未処理', false, file.getLastUpdated()
            ]);
            ...
            try {
              ...
              errorMessages.push(`【移動失敗】${targetFolderName}: ${moveError.message}`);
            }
            totalFilesProcessed++;
          }
        }
        ...
        if (errMsg.includes("Service error: Drive")) {
          ...
          Logger.log(`一時的なサービスエラーを無視しました（フォルダ: ${targetFolderName}）`);
          continue; 
        }
        
        if (errMsg.includes("A shared drive item must have exactly one parent") || 
            errMsg.includes("Insufficient permissions")) {
          errorMessages.push(`【権限不足】「${targetFolderName}」を確認してください。`);
        } else {
          // それ以外の予期せぬエラーは通知に含める
          errorMessages.push(`【フォルダエラー】「${targetFolderName}」: ${errMsg}`);
        }
      }
    }

    if (allResultRows.length > 0) {
      outputSheet.getRange(outputSheet.getLastRow() + 1, 1, allResultRows.length, allResultRows[0].length).setValues(allResultRows);
    }
...
function getTargetFoldersFromSheet(ss, sheetName, headerName) {
  ...
  if (colIndex === -1) throw new Error(`シート「${sheetName}」に「${headerName}」が見つかりません。`);
...
function prepareOutputSheet(ss, sheetName) {
  ...
  if (sheet.getLastRow() === 0) {
    const headers = ['音声ファイルID', '基本情報ID', '検索したフォルダ名', 'GAS検索した日時', '音声ファイル名', '音声ファイルURL', 'ステータス', 'フラグ', 'ファイルの最新更新日時'];
