...
function syncDriveFoldersToSalesforce() {
  ...
  const soql =
    "SELECT Id, OfficeName__c, GoogleFolderName__c FROM CustomerStatus__c WHERE GoogleURL__c = null AND OfficeName__c != null AND GoogleFolderName__c != null LIMIT 90";
  ...
  try {
    ...
    console.log(
      `${records.length} 件のレコードを取得しました。フォルダ作成処理を開始します...`,
    );
    ...
    records.forEach((record) => {
      ...
      try {
        ...
        console.log(
          `ID: ${record.Id} (利用者: ${googleFolderName}) のURLを更新します。`,
        );
    ...
    console.error(
      "全体処理でエラーが発生しました (SOQLエラー等の可能性): " + e.message,
    );
  }
}
