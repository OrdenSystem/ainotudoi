...
function test_compare_v1_v2(sourceLogId) {
  ...
  try {
    ...
    if (tailLogId !== newLogId) {
      Logger.log(
        "⚠️ 最終行 LogID と受付返却 LogID が一致しません: tail=" +
          tailLogId +
          " / received=" +
          newLogId,
      );
  ...
  Logger.log(
    "比較対象: source LogID=" + sourceLogId + " / new LogID=" + testLogId,
  );
  Logger.log("現行アクティブ版: " + HOPE_RECORDER_ACTIVE.version);
  Logger.log("完了後に M 列（文字起こしテキスト）同士を比較してください");
  Logger.log(
    "V1/V2 比較したい場合は 100_HopeRecorderPrompts.js の HOPE_RECORDER_ACTIVE を切り替えて再度 test_compare_v1_v2 を実行",
  );
...
function test_run_v2_only(driveUrl) {
  ...
  const createDate = Utilities.formatDate(
    new Date(),
    "Asia/Tokyo",
    "yyyy/MM/dd",
  );
...
function test_audioMetadata(driveUrl) {
  ...
  Logger.log(
    "長尺（60分超）判定: " + (isLong ? "YES（Slack 警告対象）" : "NO"),
  );
}
 ...
 */
const TEST_AUDIO_URL_2026_04_27 =
  "https://drive.google.com/file/d/1li9NxKjoQXNjDagl0NiyzQ9PAPXA9TVE/view?usp=sharing";
