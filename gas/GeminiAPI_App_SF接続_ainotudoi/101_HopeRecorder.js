...
function _incrementAndCheckRetryLimit_(sheet, rowNumber, logId, reason) {
  ...
  if (newCount > MAX_RETRY_COUNT) {
    ...
    sheet.getRange(rowNumber, 5).setValue('リトライ上限(' + MAX_RETRY_COUNT + ')超過: ' + String(reason || '').substring(0, 200));
    sendSlackNotification('🚨 [HopeRecorder] LogID:' + logId + ' リトライ上限(' + MAX_RETRY_COUNT + ')超過。手動介入を要請。');
    console.error('[' + logId + '] retry limit exceeded N=' + newCount);
    return true;
  }
  console.warn('[' + logId + '] retry count: ' + newCount + '/' + MAX_RETRY_COUNT);
  return false;
}
...
// 受付➀：AppSheet → GAS
// ============================================
function HopeRecorderStartTranscription(audioFileUrl, audioFileName, saleseforceUserID, saleseforceObject, Record_StartTime__c, CreateDate__c) {
  ...
  if (!audioFileUrl || !String(audioFileUrl).trim()) {
    var rejectMsg = '[HopeRecorder] 受付拒否: audioFileUrl 空。'
      + ' 呼出元 AppSheet の設定（音声ファイル経路 / モバイル HopeRecorder / 別 Bot のいずれか）を要確認。'
      + ' params={audioFileName:' + (audioFileName ? 'set' : 'empty')
      + ', saleseforceUserID:' + (saleseforceUserID ? 'set' : 'empty')
      + ', saleseforceObject:' + (saleseforceObject ? 'set' : 'empty')
      + ', Record_StartTime__c:' + (Record_StartTime__c ? 'set' : 'empty')
      + ', CreateDate__c:' + (CreateDate__c ? 'set' : 'empty') + '}';
    console.warn(rejectMsg);
    try { sendSlackNotification(rejectMsg); } catch (_) {}
  ...
  sheet.appendRow([
    logId, new Date(), "", "受付", "", // A-E
    audioFileUrl, audioFileName, saleseforceUserID, saleseforceObject, Record_StartTime__c, CreateDate__c, // F-K
    "", "", 0 // L,M,N
  ]);
...
function processTranscriptionWorker() {
  ...
  if (!GCS_BUCKET_NAME || !PROJECT_ID) {
    console.warn('[HopeRecorder] GCS_BUCKET_NAME または PROJECT_ID が未設定のため processTranscriptionWorker を停止します');
    return '未設定のため処理しません';
  }
  const sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName(SHEET_NAME);
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    try {
      ...
      const estimatedDurationSec = estimateAudioDurationSeconds_(fileSize, mimeType);
      ...
      // 大容量アップロード
      uploadLargeFileToGcsByDriveApi_(fileId, fileSize, mimeType, GCS_BUCKET_NAME, audioPath, accessToken);
      ...
      const requestJsonl = JSON.stringify({
        request: {
          contents: [{ role: "user", parts: [{ fileData: { mimeType: mimeType, fileUri: gcsUri } }, { text: prompt }] }],
          safetySettings: [
      ...
      const jsonlUploadResponse = UrlFetchApp.fetch(
        ...
        { method: "post", contentType: "application/json", payload: requestJsonl, headers: { Authorization: `Bearer ${accessToken}` }, muteHttpExceptions: true }
      );
      if (jsonlUploadResponse.getResponseCode() !== 200) throw new Error("JSONLアップロード失敗");
      ...
      const batch = UrlFetchApp.fetch(
        `https://${LOCATION}-aiplatform.googleapis.com/v1/projects/${PROJECT_ID}/locations/${LOCATION}/batchPredictionJobs`,
        {
          ...
          payload: JSON.stringify({
            ...
            inputConfig: { instancesFormat: "jsonl", gcsSource: { uris: [`gs://${GCS_BUCKET_NAME}/${jsonlPath}`] } },
            outputConfig: { predictionsFormat: "jsonl", gcsDestination: { outputUriPrefix: `gs://${GCS_BUCKET_NAME}/outputs/${logId}/` } },
            model: `projects/${PROJECT_ID}/locations/${LOCATION}/publishers/google/models/${HOPE_RECORDER_ACTIVE.batch.model}`,
            modelParameters: { speechConfig: HOPE_RECORDER_ACTIVE.batch.speechConfig }
          }),
      ...
      if (batch.getResponseCode() !== 200) {
        ...
        console.error('[' + logId + '] Batch投入失敗 HTTP ' + code + ': ' + batch.getContentText());
      ...
      sheet.getRange(rowNumber, 12).setValue(`gs://${GCS_BUCKET_NAME}/outputs/${logId}/`);
      ...
      if (errMsg.includes("Service error")) {
        ...
        if (_incrementAndCheckRetryLimit_(sheet, rowNumber, logId, 'Service error: ' + errMsg)) {
  ...
  if (errorMessages.length > 0) {
    sendSlackNotification(`🚨 **Batch投入エラー発生**\n${errorMessages.join('\n')}`);
  }
...
// 大容量アップローダー (内部関数)
// ============================================
function uploadLargeFileToGcsByDriveApi_(fileId, fileSize, mimeType, bucket, path, gcsAccessToken) {
  ...
  if (initResp.getResponseCode() !== 200) throw new Error(`GCS初期化失敗: ${initResp.getContentText()}`);
  ...
  while (offset < fileSize) {
    ...
    const downloadHeaders = { "Authorization": `Bearer ${driveAccessToken}`, "Range": `bytes=${offset}-${end - 1}` };
    const downloadResp = UrlFetchApp.fetch(downloadUrl, { headers: downloadHeaders, muteHttpExceptions: true });

    if (downloadResp.getResponseCode() !== 200 && downloadResp.getResponseCode() !== 206) throw new Error(`Driveダウンロード失敗`);
    ...
    const uploadResp = UrlFetchApp.fetch(uploadUrl, {
      method: "put", payload: partialBlob, headers: { "Content-Range": rangeHeader }, muteHttpExceptions: true
    });

    const code = uploadResp.getResponseCode();
    if (code !== 308 && code !== 200 && code !== 201) throw new Error(`アップロード中断: ${code}`);
    offset = end;
  }
}
...
function checkJobStatus() {
  // 環境固有値ガード: GCS バケット / GCP プロジェクトが未設定の環境では誤発火を防ぐために停止
  if (!GCS_BUCKET_NAME || !PROJECT_ID) {
    console.warn('[HopeRecorder] GCS_BUCKET_NAME または PROJECT_ID が未設定のため checkJobStatus を停止します');
    return;
  }
  const sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName(SHEET_NAME);
  const values = sheet.getDataRange().getValues();
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (remainingMs < CHECK_JOB_STATUS_REMAINING_THRESHOLD_MS_) {
      skippedCount++;
      console.info('[checkJobStatus] 残時間 ' + remainingMs + 'ms < ' + CHECK_JOB_STATUS_REMAINING_THRESHOLD_MS_ + 'ms。LogID=' + logId + ' を次回持ち越し（経過 ' + elapsedMs + 'ms）');
      continue;
    }
    ...
    try {
      ...
      if (job.state === "JOB_STATE_SUCCEEDED") {
        ...
        (list.items || [])
          ...
          .forEach(obj => {
            ...
            const content = UrlFetchApp.fetch(url, { headers: { Authorization: `Bearer ${token}` } }).getContentText();
            content.trim().split(/\r?\n/).forEach(line => {
              try {
                const t = JSON.parse(line)?.response?.candidates?.[0]?.content?.parts?.[0]?.text;
                if (t) rawDraftText += t + "\n";
              } catch (_) { }
            });
          });
        ...
        if (rawDraftText.trim()) {
          ...
          const finalCleanedText = callGeminiProToCleanText_(rawDraftText.trim());
      ...
      } else if (job.state === "JOB_STATE_FAILED" || job.state === "JOB_STATE_CANCELLED") {
  ...
  if (errorMessages.length > 0) {
    sendSlackNotification(`🚨 **整形・確認エラー発生**\n${errorMessages.join('\n')}`);
  }
  if (skippedCount > 0) {
    console.info('[checkJobStatus] 時間切れ持ち越し: ' + skippedCount + ' 件（次回 trigger で再処理）');
  }
}
...
function callGeminiProToCleanText_(rawText) {
  ...
  try {
    ...
    console.info('[callGeminiProToCleanText_] piiMasking entityCount=' + registry.count);
  } catch (e) {
    console.error('[callGeminiProToCleanText_] PII マスキング初期化失敗、原文で続行: ' + e.message);
    sendSlackNotification('🚨 [PiiMasker] HopeRecorder Cleaning マスキング初期化失敗。職員名が平文で Gemini に送信されている可能性があります。詳細は GAS Cloud Logging を確認してください。');
    registry = null;
  }
  var maskedRawText = registry ? maskText_(rawText, registry) : rawText;

  var cleaningPrompt = HOPE_RECORDER_ACTIVE.cleaning.promptSystem + maskedRawText;
  ...
  try {
    parsed = callGeminiWithKeyRotation_(
      primaryModel,
      payload,
      { apiVersion: 'v1beta', maxAttempts: 2 }
    );
  } catch (e) {
    primaryError = e;
    console.warn('[callGeminiProToCleanText_] pro (' + primaryModel + ') 全試行失敗: ' + e.message);
  }
  ...
  if (parsed) {
    ...
    if (!primaryText) {
      ...
      console.warn('[callGeminiProToCleanText_] pro 応答が空 finishReason=' + primaryFinishReason + '、flash フォールバックを試行');
      primaryError = new Error('Gemini クリーニング応答が空 (finishReason: ' + primaryFinishReason + ')');
      parsed = null;
    }
  }
  ...
  if (!parsed && elapsedAfterPrimary < CLEANING_TIME_BUDGET_MS_ - 30 * 1000) {
    console.info('[callGeminiProToCleanText_] flash フォールバック開始（経過 ' + elapsedAfterPrimary + 'ms / 予算 ' + CLEANING_TIME_BUDGET_MS_ + 'ms）');
    try {
      // flash 単独試行も maxAttempts:2 に短縮（残時間内で確実に終わらせる）
      parsed = callGeminiWithKeyRotation_(
        CLEANING_FALLBACK_MODEL_,
        payload,
        { apiVersion: 'v1beta', maxAttempts: 2 }
      );
      usedFallback = true;
      ...
      console.error('[callGeminiProToCleanText_] flash フォールバックも失敗: ' + e2.message);
      // 両方失敗 → throw（呼出元で catch して当該行のみエラー化）
      var combinedMsg = 'cleaning 全試行失敗: pro=' + (primaryError ? primaryError.message : 'unknown')
        + ' / flash=' + e2.message;
  ...
  if (!parsed) {
    ...
    var msg = 'cleaning time budget 超過 (' + hardElapsed + 'ms / ' + CLEANING_TIME_BUDGET_MS_ + 'ms): '
  ...
  if (!text) {
    ...
    throw new Error('Gemini クリーニング応答が空 (finishReason: ' + reason + ')');
  }

  // 最終的に hard cap 超過していた場合は警告ログのみ（応答自体は使う）
  var totalElapsed = Date.now() - cleaningStartMs;
  if (totalElapsed > CLEANING_TIME_BUDGET_MS_) {
    console.warn('[callGeminiProToCleanText_] time budget 超過後に応答取得 ' + totalElapsed + 'ms（次回以降の改善検討対象）');
  }
  ...
  if (usedFallback) {
    finalText = '[フォールバック: ' + CLEANING_FALLBACK_MODEL_ + ' 使用]\n\n' + finalText;
  }
  return finalText;
}
...
function getStaffListCachedForCleaning_() {
  ...
  try {
    ...
    console.warn('[getStaffListCachedForCleaning_] フォールバック取得失敗: ' + e.message);
    return [];
  }
}
...
function processAppSheetSendWorker() {
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    try {
      ...
      let formattedStartTime = r[9] ? Utilities.formatDate(new Date(r[9]), "Asia/Tokyo", "HH:mm:ss") : "";
      let formattedCreateDate = r[10] ? Utilities.formatDate(new Date(r[10]), "Asia/Tokyo", "yyyy/MM/dd") : "";
      ...
      const resp = UrlFetchApp.fetch(url, {
        method: 'post', contentType: 'application/json', headers: { 'ApplicationAccessKey': apiKey },
      ...
      if (resp.getResponseCode() === 200) {
        ...
        console.error('[' + logId + '] AppSheet 返却エラー HTTP ' + resp.getResponseCode() + ': ' + resp.getContentText());
      ...
      if (e.message.includes("Service error")) {
        if (_incrementAndCheckRetryLimit_(sheet, rowNumber, logId, 'AppSheet Service error: ' + e.message)) {
  ...
  if (errorMessages.length > 0) {
    sendSlackNotification(`🚨 **AppSheet送信エラー発生**\n${errorMessages.join('\n')}`);
  }
}
...
function estimateAudioDurationSeconds_(fileSize, mimeType) {
  if (!fileSize || fileSize <= 0) {
    console.warn('[estimateAudioDuration] fileSize が 0 以下。破損ファイルの可能性: ' + fileSize);
    return 0;
  }
  if (!mimeType) {
    console.warn('[estimateAudioDuration] mimeType 未定義のため圧縮系ビットレートで近似します');
  }
