...
function getStaffList() {
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      ...
      if (backup) {
        console.log("AppSheet APIエラーのため、金庫(バックアップ)から職員リストを復元します");
    ...
    const staffData = rows.map(r => {
      const name = r["NameKana__c"] || r["Name__c"] || r["氏名"] || r["Name"] || "名称不明";
...
function callAppSheetApi(appId, apiKey, tableName, selector) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (code === 200) {
        try {
          ...
          failureLog.push(`試行${attempt}: JSONパース失敗 (${parseErr.message})`);
        }
      } else if (code === 429) {
        isRateLimit = true;
    ...
    if (attempt < MAX_RETRIES) {
      ...
      const waitTime = isRateLimit ? (5000 * attempt + jitter) : (2000 * attempt + jitter);
      console.log(`[AppSheet API] ${tableName} 取得失敗。${waitTime/1000}秒後に再試行します...`);
      Utilities.sleep(waitTime);
    }
  }

  console.error(`AppSheet API 呼び出し失敗\nテーブル: ${tableName}\n${failureLog.join('\n')}`);
  try {
    notifyError("callAppSheetApi", `${tableName} 取得失敗 (4回試行)\n` + failureLog.join('\n'));
  } catch (ne) {}
