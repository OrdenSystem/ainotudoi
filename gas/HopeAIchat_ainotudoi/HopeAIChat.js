...
function getCachedUser_(props, userId) {
  ...
  try {
    ...
    const rows = callAppSheetApi(props.getProperty("APPSHEET_APP_ID"), props.getProperty("APPSHEET_API_KEY"), "CustomerStatus__c", selector);
...
function processUserMessage(userId, userMessage, fileData, useContext, frontendHistory) {
  try {
    ...
    if (!user) return "利用者情報が取得できませんでした。画面を再読み込みしてください。";
    ...
    const reply = callGeminiApi(props.getProperty("API_KEY"), contextText, frontendHistory, userMessage, fileData);
    // FIXED: [R3] エラー応答時は履歴保存をスキップ
    if (!reply.startsWith("申し訳ございません")) {
      saveChatHistoryToDrive(user, userId, fileData ? `[添付あり] ${userMessage}` : userMessage, reply);
    }
    return reply;
  } catch (e) {
    // FIXED: [B4] エラー通知をSlackに送信 + 個人情報を含まない汎用メッセージを返却
    console.error("processUserMessage Error: " + e.message);
    try { notifyError("processUserMessage", e.message); } catch(ne) {}
...
function analyzeHistoryForPrompts_(historyText) {
  ...
  Object.entries(tagFreq).sort((a, b) => b[1] - a[1]).slice(0, 3).forEach(e => results.push(e[0]));
  ...
  lines.forEach(line => {
    ...
    if (m) {
      let msg = m[1].trim();
      // 指示オプション部分を除去
      msg = msg.split('=== 指示オプション')[0].split('=== トーン指示')[0].trim();
      // 添付タグ除去
  ...
  userMessages.forEach(msg => {
    ...
    if (msg.length < patternFreq[key].shortest.length) patternFreq[key].shortest = msg;
  });
...
function readFileFromChatPDFFolder(userObj, suffix) {
  try {
    ...
    while (files.hasNext()) {
      const f = files.next();
      if (f.getName().replace(/\s+/g, "").includes(cleanUserName) && f.getName().includes(suffix)) return f.getBlob().getDataAsString("UTF-8");
    }
    return null;
  } catch (e) { return null; }
}
...
function getContextFileContent(userObj, userId) {
  ...
  try {
    ...
    while (files.hasNext()) {
      const f = files.next();
      if (f.getName().replace(/\s+/g, "").includes(cleanUserName) && f.getName().includes("AIコンテキスト")) {
        fileList.push({ name: f.getName(), content: f.getBlob().getDataAsString("UTF-8") });
      }
    }
    fileList.sort((a,b) => a.name.localeCompare(b.name)).forEach(f => combinedText += `\n\n=== 参照: ${f.name} ===\n\n` + f.content);
  } catch(e){}
...
function saveChatHistoryToDrive(userObj, userId, q, a) {
  try {
    ...
    let file, current = "";
    if (files.hasNext()) { file = files.next(); current = file.getBlob().getDataAsString(); }
    ...
    if (news.length > 80000) {
      ...
      const archiveContent = news.substring(0, splitPoint > 0 ? splitPoint : half);
      ...
      if (file) file.setContent(keepContent); else folder.createFile(fileName, keepContent, MimeType.PLAIN_TEXT);
      if (keepContent.length < 90000) CacheService.getScriptCache().put(`HIST_${userId}`, keepContent, 3600);
    } else {
      if (file) file.setContent(news); else folder.createFile(fileName, entry, MimeType.PLAIN_TEXT);
      if (news.length < 90000) CacheService.getScriptCache().put(`HIST_${userId}`, news, 3600);
    }
  } catch(e){ console.error("saveChatHistory Error: " + e.message); } // FIXED: [R4] catch にログ出力
}
...
function callGeminiApi(apiKey, context, frontendHistory, message, fileData) {
  ...
  const models = primaryModel !== fallbackModel ? [primaryModel, fallbackModel] : [primaryModel];
  ...
  // ペイロード組み立て
  const contents = [{ role: "user", parts: [{ text: "あなたは福祉事業所の支援アシスタントです。最新の会話履歴を優先してください。\n\n【知識】\n" + context }] }];
  contents.push({ role: "model", parts: [{ text: "了解しました。" }] });
  if (frontendHistory) frontendHistory.forEach(m => contents.push({ role: m.role, parts: [{ text: m.text }] }));
  const userParts = [{ text: message }];
  if (fileData) userParts.push({ inline_data: { mime_type: fileData.type, data: fileData.data } });
  ...
  for (let keyIdx = 0; keyIdx < apiKeys.length; keyIdx++) {
    ...
    for (const model of models) {
      ...
      let resBody;
      try { resBody = JSON.parse(resText); } catch (parseErr) {
      ...
      if (resCode === 200) {
        if (!resBody.candidates || resBody.candidates.length === 0) {
          ...
          console.warn(`[callGeminiApi] ${keyLabel}/${model} candidates empty: ${reason}`);
          blockReason = reason;
      ...
      // エラー処理
      const errDetail = maskSecrets_(resBody.error?.message || resText.substring(0, 200));
      console.warn(`[callGeminiApi] ${keyLabel}/${model} HTTP ${resCode}: ${errDetail}`);
      allErrors.push(`${keyLabel}/${model} HTTP ${resCode}: ${errDetail}`);

      // quota/429/帯域幅 エラー判定 → 次のAPIキーへスキップ
      const isQuota = (resCode === 429) ||
  ...
  // 全失敗
  try {
    notifyError("callGeminiApi", `Gemini API 呼び出し全失敗 (キー${apiKeys.length}本×モデル${models.length}本試行)\n` + allErrors.join("\n"));
  } catch (ne) {}
...
function registerCaseRecord(userId, content, staffId, consultId) {
  ...
  const userName = (user && user["CustomerName__c"]) ? String(user["CustomerName__c"]) : "";

  let staffName = "";
  try {
    const staffList = getStaffList();
    if (staffList && staffList.length) {
      const hit = staffList.find(function(s) { return s.id === staffId; });
  ...
  let userMail = "";
  try { userMail = Session.getActiveUser().getEmail() || ""; } catch (me) {}
  ...
  if (useJdbc) {
    try {
      ...
      console.warn("[registerCaseRecord] JDBC失敗、AppSheet APIにフォールバック: " + e.message);
  ...
  try { notifyError("registerCaseRecord", maskedDetail); } catch (ne) {}
...
function maskId_(id) {
  ...
  return s.substring(0, 4) + "*".repeat(Math.max(4, s.length - 8)) + s.substring(s.length - 4);
}
...
function registerCaseRecordJDBC_(userId, content, staffId, consultId, options) {
  ...
  try {
    ...
    var now = new Date();
    var nowJst = Utilities.formatDate(now, 'Asia/Tokyo', "yyyy-MM-dd HH:mm:ss") + "+00";
...
function registerCaseRecordAppSheet_(userId, content, staffId, consultId) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (attempt === MAX_RETRIES) {
      throw new Error(`AppSheet API Error (HTTP ${code}): ${response.getContentText().substring(0, 200)}`);
    }
...
function testRegisterCaseRecordJDBC() {
  ...
  let userMail = "";
  try { userMail = Session.getActiveUser().getEmail() || ""; } catch (e) {}
  ...
  const result = registerCaseRecordJDBC_(
    ...
    staffId,
    null,
    { officeId: officeId, userName: userName, staffName: staffName, userMail: userMail }
  );
