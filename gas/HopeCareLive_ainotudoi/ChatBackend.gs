...
function getInitialChatState(targetId) {
  ...
  try {
    const rows = callAppSheetApi(
      ...
      APPSHEET_API_KEY,
      "CustomerStatus__c",
      `Filter(CustomerStatus__c, [Row ID] = '${targetId}')`,
    );
    if (rows.length === 0) {
      result.error = "利用者が見つかりません。";
      return result;
    }
...
function processUserMessage(userId, userMessage, fileData) {
  try {
    ...
    if (!userRows || userRows.length === 0)
      throw new Error("利用者が特定できません");
    const user = userRows[0];

    const contextText = getContextFileContent(user, userId);
    if (!contextText)
      return "⚠️ データファイルがありません。AppSheetで生成してください。";
    ...
    const logMsg = fileData
      ? `[ファイル添付: ${fileData.name}] ${userMessage}`
      : userMessage;
...
function callGeminiApiOnce_(url, context, history, message, fileData) {
  ...
  promptText +=
    "以下の「利用者データ」および「これまでの会話履歴」に基づいて、ユーザーの質問に答えてください。\n";
  promptText +=
    "回答は支援者に向けて、客観的かつ分かりやすく、Markdown形式で見やすく整形してください。\n";
  if (fileData)
    promptText +=
      "※ユーザーから画像またはPDFファイルが添付されました。その内容も踏まえて回答してください。\n";
  ...
  try {
    ...
    if (folder) {
      ...
      while (files.hasNext()) {
        const f = files.next();
        if (
          f.getMimeType() === MimeType.PLAIN_TEXT &&
