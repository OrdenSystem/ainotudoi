function doPost(e) {
  try {
    ...
    if (action === "getUserContext") {
      ...
      result = {
        jsonString: previewSummary(
          postData.rawText,
          postData.contextInfo,
          postData.staffId,
        ),
      };
    } else if (action === "registerFinalData") {
      result = registerFinalData(
        postData.finalFactText,
        postData.emotionText,
        postData.startTime,
        postData.appSheetIds,
      );
    }
