...
function getUserContext(riyousyaId) {
  try {
    ...
    while (files.hasNext()) {
      ...
      if (
        fileName.includes("AIコンテキスト") &&
        fileName.includes("最新") &&
...
function processAudioDebug(base64Data, contextPayload) {
  ...
  if (!DEEPGRAM_API_KEY)
    return { httpCode: 500, error: "DEEPGRAM_API_KEY未設定" };
  ...
  if (rawContext) {
    ...
    while ((match = nameRegex.exec(headerContext)) !== null) {
      if (match[1]) {
        ...
        if (kanjiName && kanjiName.length >= 2)
          keywordsArray.push(`${kanjiName}:2`);
      }
      if (match[2]) {
        // フリガナ（スペースを詰める）
        const kanaName = match[2].replace(/\s+/g, "").trim();
        if (kanaName && kanaName.length >= 2)
          keywordsArray.push(`${kanaName}:2`);
      }
    }
  ...
  if (encodedKeywords.length > 1000) {
    encodedKeywords = encodeURIComponent(
      userName ? `${userName.replace(/\s+/g, "")}:3` : "",
    );
  }

  let dgUrl =
    "https://api.deepgram.com/v1/listen?model=nova-3&language=ja&smart_format=true&punctuate=true&filler_words=false";
  if (encodedKeywords) {
    ...
    if (rawContext) {
      ...
      while ((match = nameRegex.exec(headerContext)) !== null) {
        if (match[1]) {
          ...
          if (n.length >= 2) keywords.push(n + ":2");
        }
        if (match[2]) {
          const n = match[2].replace(/\s+/g, "").trim();
          if (n.length >= 2) keywords.push(n + ":2");
        }
      }
    }
