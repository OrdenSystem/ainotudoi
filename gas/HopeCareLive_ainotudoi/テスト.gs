function testGetContextFlow() {
  ...
  try {
    const chatPdfFolders =
      DriveApp.getFolderById(folderId).getFoldersByName("ChatPDF");
    ...
    while (files.hasNext()) {
      ...
      if (fileName.includes("AIコンテキスト")) {
        ...
        try {
          ...
          console.log(
            `✅ [パターンA] 標準ルートで読み込み成功！文字数: ${content.length}`,
          );
        } catch (e) {
          console.log(`⚠️ [パターンA] 失敗: ${e.toString()}`);
          console.log(
            `🔄 [パターンB] Drive API (UrlFetchApp)での強制読み込みを試します...`,
          );
          ...
          if (res.getResponseCode() === 200) {
            ...
            console.log(
              `✅ [パターンB] 強制ルートで読み込み成功！！文字数: ${contentB.length}`,
            );
          } else {
            console.log(
              `❌ [パターンB] も失敗しました... 原因: ${res.getContentText()}`,
            );
          }
        }
      }
