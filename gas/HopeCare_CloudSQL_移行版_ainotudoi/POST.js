...
function postFolderUrlToExternalApp(newFolderUrl, zaisekiID) {
  ...
  if (!endpoint || !token) {
    console.error('postFolderUrlToExternalApp: SF_WEBAPP_URL または WEBAPP_SHARED_TOKEN が未設定です');
    return;
  }
  ...
  try {
    ...
    console.log(`外部連携結果: ${response.getResponseCode()} / 内容: ${responseBody}`); // 中身もログに出す
  } catch (e) {
    console.error(`外部連携エラー: Salesforceへの連携に失敗しました。詳細: ${e.toString()}`);
  }
}
...
function test_doPostSecurity_Ping() {
  ...
  var pings = [
    { label: '① 正規トークン (success 期待)',     payload: { function: 'ping', token: token },                    expectSuccess: true  },
    { label: '② 不正トークン (Unauthorized 期待)', payload: { function: 'ping', token: 'invalid-token-xxx-xxx' }, expectSuccess: false },
    { label: '③ トークンなし (Unauthorized 期待)', payload: { function: 'ping' },                                  expectSuccess: false }
  ];
  ...
  pings.forEach(function(t) {
    ...
    var parsed;
    try { parsed = JSON.parse(body); } catch (e) { parsed = { _raw: body }; }
    var ok = t.expectSuccess ? !!parsed.success : (!!parsed.error && parsed.error === 'Unauthorized');
  ...
  if (failed > 0) {
    Logger.log('⚠️ 期待通りに動作していません。受信側のデプロイが最新版か確認してください（バージョンを「新しいバージョン」で再デプロイ）。');
  } else {
