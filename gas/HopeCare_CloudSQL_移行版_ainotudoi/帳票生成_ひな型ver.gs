 ...
 * =================================================================================
 */
function originalFileMake_async(
  surceFileUrl,
  driveURL,
  textPaste,
  zaisekiID,
  ...
  if (!googleFolderName)
    missingParams.push("Googleフォルダ名 (googleFolderName)");
  ...
  try {
    ...
    if (!targetFolder) {
      ...
      let customerFolder;
      try {
        customerFolder = DriveApp.getFolderById(
          customerDriveURL.match(/[-\w]{25,}/)[0],
        );
      } catch (e) {
      ...
      console.log(
        `[Folder] 利用者フォルダ「${googleFolderName}」の作成を待機・検索します...`,
      );
      ...
      for (let i = 0; i < maxRetries; i++) {
        ...
        if (targetFolders.hasNext()) {
          ...
          console.log(
            `[Wait] まだ見つかりません... AppSheetの作成を待ちます (${i + 1}/${maxRetries})`,
          );
      ...
      console.log(
        `[External] 発見したフォルダURLをSalesforceへ通知します (ID: ${zaisekiID})`,
      );
  ...
  if (lock.tryLock(30000)) {
    ...
    try {
      ...
      const colIdx = {
        ...
        textPaste: header.indexOf("textPaste"),
        fileUrl: header.indexOf("fileUrl"),
        fileName: header.indexOf("fileName"),
        updatedAt: header.indexOf("updatedAt"),
        errorMessage: header.indexOf("errorMessage"),
        jobId: header.indexOf("jobId"),
      };
      if (colIdx.status === -1)
        throw new Error("必須ヘッダー「status」列が見つかりません。");

      // --- 2. 処理対象ジョブの選定 ---
      const fifteenMinutesAgo = new Date(Date.now() - 15 * 60 * 1000);
      const jobsToProcess = values
        .map((row, index) => ({ rowData: row, rowIndex: index + 2 }))
        .filter((job) => {
          const status = job.rowData[colIdx.status];
          const updatedAt = new Date(job.rowData[colIdx.updatedAt]);
          return (
            status === "PENDING" ||
            (status === "PROCESSING" && updatedAt < fifteenMinutesAgo)
          );
        });
      ...
      console.log(
        `処理対象のジョブが ${jobsToProcess.length}件見つかりました。`,
      );
      ...
      for (const job of jobsToProcess) {
        if (Date.now() - startTime > TIME_LIMIT_MS) {
          console.log(
            "実行時間が上限に近づいたため処理を中断します。残りのジョブは次回に処理されます。",
          );
          break;
        }
        ...
        try {
          ...
          console.log(
            `[Job Done] ジョブ ${jobId} (${fileName}) の処理が正常に完了しました。`,
          );
        } catch (e) {
          ...
          console.error(
            `[Job Error] ジョブID: ${jobId} (${fileName}) エラー: ${errorMsg}`,
          );
      ...
      console.error(
        `[Fatal Error] ジョブ処理中に致命的なエラー: ${e.toString()}`,
      );
      // sendSlackNotification(...)
    } finally {
      lock.releaseLock();
