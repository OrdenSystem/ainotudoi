...
function completeBillingWithIDs_CloudSQL(targetIDs, backupFolderUrl) {
  ...
  try {
    ...
    try {
      ...
      var header01 = [];
      for (var ci = 1; ci <= colCount; ci++) header01.push(meta.getColumnName(ci));
      rs.close();
      stmt.close();

      // 対象IDのレコードを取得（更新前バックアップ用）
      var placeholders = idList.map(function() { return '?'; }).join(', ');
      stmt = conn.prepareStatement('SELECT * FROM "01相談記録" WHERE "相談記録ID" IN (' + placeholders + ')');
      ...
      if (targetRowsBefore.length === 0) {
        ...
        // 2. 【更新前】CSV保存
        saveRowsToCsv_CloudSQL(targetRowsBefore, header01, `BEFORE_COMPLETE_${ts}.csv`, folder);
        log(`更新前CSV保存完了 (${targetRowsBefore.length}件)`);

        // 3. ★ フラグの更新処理（JDBC UPDATE）
        conn.setAutoCommit(false);
        stmt = conn.prepareStatement('UPDATE "01相談記録" SET "フラグ" = TRUE WHERE "相談記録ID" = ?');
        ...
        while (rs.next()) {
          var aRow = [];
          for (var aj = 1; aj <= colCount; aj++) aRow.push(rs.getString(aj) || "");
          targetRowsAfter.push(aRow);
        }
        rs.close();
        stmt.close();

        saveRowsToCsv_CloudSQL(targetRowsAfter, header01, `AFTER_COMPLETE_${ts}.csv`, folder);
        log(`更新後CSV保存完了`);
      }
    } finally {
...
function saveRowsToCsv_CloudSQL(rows, header, fileName, folder) {
  const csvString = [header, ...rows].map(row => {
    return row.map(cell => {
