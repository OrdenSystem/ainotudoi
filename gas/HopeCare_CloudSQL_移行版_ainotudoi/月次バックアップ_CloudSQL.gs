...
function executeMonthlyBackup_(yyyymm) {
  ...
  try {
    ...
    var targetFolder = folderIter.hasNext()
      ? folderIter.next()
      : parentFolder.createFolder(yyyymm);
    ...
    sendSlackNotification(
      "❌ 月次CSVバックアップエラー (" + yyyymm + "): " + e.message,
    );
  }
}
  ...
  try {
    ...
    while (rs.next()) {
      ...
      for (var j = 1; j <= colCount; j++) {
        cells.push(
          '"' + String(rs.getString(j) || "").replace(/"/g, '""') + '"',
        );
      }
