...
function setupQueueSheet() {
  ...
  if (sheet.getLastRow() === 0) {
    ...
    Logger.log(
      `ℹ️ シート '${QUEUE_SHEET_NAME}' には既にデータがあるため、ヘッダー追加をスキップしました。`,
    );
  }
}
...
function enqueueAIContextTask(targetId) {
  ...
  sheet.appendRow([targetId, "PENDING", new Date(), "", "", queueId]);
...
function processAIContextQueue() {
  ...
  try {
    ...
    try {
      ...
      if (APP_ID && API_KEY) {
        cachedStaffList = callAppSheetApi(
          APP_ID,
          API_KEY,
          "StaffStatus__c",
          "",
      ...
      Logger.log(
        `⚠️ 職員マスタキャッシュ失敗（各ユーザー処理内で個別取得します）: ${e.message}`,
      );
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (
        ...
        try {
          ...
          Logger.log(
            `▶️ 処理開始: ID=${targetId} (Row=${rowIndex}, Status=${status}, QueueID=${queueId})`,
          );
          ...
          generateAIContextFile(targetId, {
            isFirstRun: isReset,
            cachedStaffList: cachedStaffList,
          });
    ...
    try {
      ...
      if (freshLastRow >= 2) {
        ...
        if (retryCount > 0)
          Logger.log(
            `🔁 ERROR→RETRY変更: ${retryCount}件（次回トリガーで再処理）`,
          );
      }
    } catch (retryErr) {
    ...
    try {
      ...
      if (cleanLastRow >= 2) {
        ...
        for (let i = 0; i < cleanData.length; i++) {
          ...
          if (
            cStatus === "COMPLETED" &&
            cCreatedAt instanceof Date &&
            cCreatedAt < sevenDaysAgo
          ) {
        ...
        if (rowsToDelete.length > 0) {
          ...
          const existingFolders =
            parentFolder.getFoldersByName(backupFolderName);
          ...
          Logger.log(
            `💾 CSVバックアップ保存: ${fileName} (${rowsToDelete.length}件)`,
          );
