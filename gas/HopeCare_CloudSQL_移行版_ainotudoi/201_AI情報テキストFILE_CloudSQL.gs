  ...
  try {
    ...
    try {
      ...
      const rowsConsult = isFirstRun
        ? selectAsObjects_(_ctxConn, "01相談記録", '"利用者在籍ID" = ?', [
            userId,
          ])
        : selectAsObjects_(
            _ctxConn,
            "01相談記録",
            '"利用者在籍ID" = ? AND "更新日時" >= ? AND "更新日時" < ?',
            [userId, start, end],
          );
      ...
      const rowsCase = isFirstRun
        ? selectAsObjects_(_ctxConn, "ケース記録", '"利用者在籍ID" = ?', [
            userId,
          ])
        : selectAsObjects_(
            _ctxConn,
      ...
      if (isFirstRun) {
        ...
        if (pIds.length > 0) {
          const ph = pIds
            ...
            .join(", ");
          rowsFormDetails = selectAsObjects_(
            _ctxConn,
            "帳票子レコード複製登録",
            '"帳票マスタ複製登録ID" IN (' + ph + ")",
            pIds,
        ...
        // 当日更新分
        const todayChild = selectAsObjects_(
          _ctxConn,
          "帳票子レコード複製登録",
          '"更新日時" >= ? AND "更新日時" < ?',
          [start, end],
        ...
        if (pIdsToday.length > 0) {
          const ph2 = pIdsToday
            ...
            .join(", ");
          const parentChild = selectAsObjects_(
            _ctxConn,
            "帳票子レコード複製登録",
            '"帳票マスタ複製登録ID" IN (' + ph2 + ")",
            pIdsToday,
