 ...
 */
function applyTextPasteToSpreadsheetUrl_CloudSQL(
  spreadsheetUrl,
  textPaste,
  reportMasterId,
) {
  ...
  try {
    ...
    // --- DBから「項目 → セル位置」を取得 ---
    const keyToPositions = getKeyToPositionsFromDb_applyToSs_CloudSQL_(
      spreadsheetUrl,
      reportMasterId,
    );
    ...
    keyToPositions.forEach((posList, key) => {
      ...
      const pasteValue = isPasteTarget
        ? String(pasteMap.get(key) ?? "").trim()
        : "";
    ...
    sheetActions.forEach((actionsMap, sheetName) => {
      ...
      actionsMap.forEach((action, a1) => {
        ...
        if (
          posIdx &&
          posIdx.row - 1 < cacheData.length &&
          posIdx.col - 1 < cacheData[0].length
        ) {
          currentRaw = String(
            cacheData[posIdx.row - 1][posIdx.col - 1] ?? "",
          ).trim();
        }
 ...
 */
function getKeyToPositionsFromDb_applyToSs_CloudSQL_(
  spreadsheetUrl,
  reportMasterId,
) {
  var conn, stmt, rs;
  ...
  try {
    ...
    if (reportMasterId) {
      sql =
        'SELECT "&&項目名&&", "シート名セル位置" FROM "帳票子レコード複製登録" WHERE "帳票マスタ複製登録ID" = ?';
      param = String(reportMasterId).trim();
    } else if (spreadsheetUrl) {
      sql =
        'SELECT "&&項目名&&", "シート名セル位置" FROM "帳票子レコード複製登録" WHERE "URL" = ?';
    ...
    while (rs.next()) {
      ...
      pos.split(",").forEach(function (p) {
        map.get(key).push(String(p).trim());
      });
    }
