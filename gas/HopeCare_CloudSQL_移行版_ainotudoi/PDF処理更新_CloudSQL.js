...
function uploadFileToGemini_CloudSQL_(fileBlob, apiKey) {
  return callWithRetry_CloudSQL_(function() {
    ...
    if (responseCode !== 200) {
      ...
      throw new Error('Geminiへのファイルアップロード失敗: ' + responseCode + ' - ' + responseText);
    }
...
function formatToSqlDate_CloudSQL_(dateVal) {
  ...
  var standardMatch = s.match(/^(\d{4})[-/年]\s*(\d{1,2})[-/月]\s*(\d{1,2})[日]?$/);
  if (standardMatch) {
    return standardMatch[1] + '-' + ('0' + standardMatch[2]).slice(-2) + '-' + ('0' + standardMatch[3]).slice(-2);
  }
  
  // 和暦の場合（令和、平成、昭和）
  var jpMatch = s.match(/(令和|平成|昭和)\s*([0-9０-９元]+)\s*年\s*([0-9０-９]+)\s*月\s*([0-9０-９]+)\s*日/);
  if (jpMatch) {
    var era = jpMatch[1];
    var yearStr = jpMatch[2];
    var year = (yearStr === '元') ? 1 : parseInt(yearStr.replace(/[０-９]/g, function(c){return String.fromCharCode(c.charCodeAt(0)-0xFEE0);}), 10);
    var month = parseInt(jpMatch[3].replace(/[０-９]/g, function(c){return String.fromCharCode(c.charCodeAt(0)-0xFEE0);}), 10);
    var day = parseInt(jpMatch[4].replace(/[０-９]/g, function(c){return String.fromCharCode(c.charCodeAt(0)-0xFEE0);}), 10);
    ...
    return gregYear + '-' + ('0' + month).slice(-2) + '-' + ('0' + day).slice(-2);
  }
...
function getAllowedOptionsByParent_CloudSQL_(parentid) {
  var masterSsId = PropertiesService.getScriptProperties().getProperty('MASTER_SS_ID');
  if (!masterSsId) throw new Error('スクリプトプロパティ MASTER_SS_ID が設定されていません');

  var SHEET_NAME = 'ひな型帳票マスタ子レコード選択肢';
  var sh = SpreadsheetApp.openById(masterSsId).getSheetByName(SHEET_NAME);
  if (!sh) throw new Error('選択肢マスタシート「' + SHEET_NAME + '」が見つかりません');
  ...
  out.forEach(function(v, k) { out.set(k, Array.from(new Set(v))); });
  return out;
}
...
function normalizeSingleSelect_CloudSQL_(aiValue, options) {
  ...
  var hit = options.find(function(o) { return o === raw; });
...
function normalizeMultiSelect_CloudSQL_(aiValue, options) {
  ...
  src.map(function(v) { return String(v == null ? '' : v).trim(); })
      .filter(Boolean)
      .forEach(function(v) {
        var hit = options.find(function(o) { return o === v; });
        if (hit) {
  ...
  if (arr.length === 0) {
    return { values: ['該当なし'], hitCount: hitCount, missCount: Math.max(missCount, 1), reason: '全ミス/空' };
  }
  return { values: arr, hitCount: hitCount, missCount: missCount, reason: '一部/全部ヒット' };
}
...
function AppsheetGeminiFileAI_CloudSQL(
  ...
  temperature, topP, tg_FileName, tg_AddFilUrl, parentid
) {
  var executionLog = [];
  var clientLog = [];
  ...
  try {
    ...
    if (parentid && String(parentid).trim() !== '') {
      ...
      executionLog.push('選択肢マスタ取得: parentid=' + parentid + ' / 項目数=' + optionCount);
    }
    ...
    try {
      ...
      throw new Error('対象フォルダのURLが無効か、アクセス権限がありません。\nURL: ' + folderURL);
    }

    clientLog.push('✅ 選択肢：項目数=' + optionCount + ' ➣ 対象フォルダ発見(' + userFolder.getName() + ')');

    // --- ファイル収集 ---
    var filesToProcess = [];

    // (A) AppSheet直接添付
    executionLog.push('[DEBUG] tg_FileName=' + tg_FileName + ', tg_AddFilUrl=' + tg_AddFilUrl);
    if (tg_FileName && tg_AddFilUrl) {
      try {
        var directFolderId = tg_AddFilUrl;
        if (tg_AddFilUrl.indexOf('id=') > -1) directFolderId = tg_AddFilUrl.split('id=')[1].split('&')[0];
        else if (tg_AddFilUrl.indexOf('/folders/') > -1) directFolderId = tg_AddFilUrl.split('/folders/')[1].split(/[/?]/)[0];
        ...
        if (directFiles.hasNext()) {
          ...
          filesToProcess.push({ name: '[直接添付] ' + file.getName(), blob: file.getBlob() });
    ...
    if (caseNamePDF) {
      var recordPdfFolders = callWithRetry_CloudSQL_(function() { return userFolder.getFoldersByName('記録PDF'); });
      if (recordPdfFolders.hasNext()) {
        var recordPdfFolder = recordPdfFolders.next(); // 最初の1つだけを取得
        executionLog.push('[DEBUG] 記録PDFフォルダ確認: ' + recordPdfFolder.getName());
        
        var caseFiles = recordPdfFolder.getFilesByName(caseNamePDF);
        if (caseFiles.hasNext()) {
          var caseFile = caseFiles.next();
          filesToProcess.push({ name: '[記録] ' + caseFile.getName(), blob: caseFile.getBlob() });
          executionLog.push('ケース記録PDFを発見: ' + caseFile.getName());
          clientLog.push('✅ ケース記録PDFを発見 ➣ 処理開始');
        } else {
          executionLog.push('[DEBUG] 記録PDF内に該当ファイルがありませんでした');
        }
      } else {
    ...
    // (C) 過去帳票PDF
    var reportFileNameList = (reportFiles || '').split(',').map(function(name) { return name.trim(); }).filter(function(name) { return name; });
    if (reportFileNameList.length > 0) {
      var reportPdfFolders = callWithRetry_CloudSQL_(function() { return userFolder.getFoldersByName('帳票PDF'); });
      if (reportPdfFolders.hasNext()) {
        ...
        reportFileNameList.forEach(function(fileName) {
          ...
          if (reportFileIterator.hasNext()) {
            ...
            filesToProcess.push({ name: '[過去帳票] ' + rFile.getName(), blob: rFile.getBlob() });
            foundCount++;
          }
        });
        if (foundCount > 0) {
    ...
    if (filesToProcess.length === 0) {
      var skipMsg = '⚠️ 対象ファイルが見つかりませんでした（処理をスキップします）';
      executionLog.push(skipMsg);
      clientLog.push(skipMsg);
      Logger.log('[DEBUG] ファイル検索失敗。デバッグログ:\n' + executionLog.join('\n'));
    ...
    clientLog.push('📤 ファイルをHopeCareAIで解析中 (' + filesToProcess.length + '件)...');
    ...
    var instructionPrompt =
      ...
      '項目リスト:\n' +
      textCategory.split(',').map(function(item) { return '- ' + item.replace('||', ' (データ型: ').trim() + ')'; }).join('\n') +
      '\n\n# 回答ルール\n' +
      '- JSON形式のみ。該当なしは "" (空文字)。\n' +
      (prompt ? '- 追加指示: ' + prompt : '');
    ...
    for (var fi = 0; fi < filesToProcess.length; fi++) {
      ...
      var simpleName = fileObj.name.startsWith('[') ? fileObj.name.split(']')[0] + ']' : '[ファイル]';
    ...
    for (var ki = 2; ki <= 5; ki++) {
      var k = PropertiesService.getScriptProperties().getProperty('API_KEY_' + ki);
    ...
    for (var attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      var url = 'https://generativelanguage.googleapis.com/v1beta/models/' + modelId + ':generateContent?key=' + currentKey;
      ...
      if (code === 200) {
        if (attempt > 0) executionLog.push('リトライ成功 (' + keyLabel + ', 試行' + (attempt + 1) + '回目)');
        break;
      }
      ...
      if (code === 503 || code === 429 || code === 500) {
        ...
        executionLog.push('AI一時エラー (' + keyLabel + ', ' + code + ') 試行' + (attempt + 1) + '/' + MAX_RETRIES + ' → リトライ...');
      ...
      throw new Error('AIリクエスト失敗: ' + code + ' ' + response.getContentText());
    }

    if (response.getResponseCode() !== 200) {
      throw new Error('AIリクエスト失敗 (全' + MAX_RETRIES + '回リトライ後): ' + lastError);
    }
    ...
    try {
      ...
      var aiContent = jsonResp.candidates && jsonResp.candidates[0] && jsonResp.candidates[0].content
        && jsonResp.candidates[0].content.parts && jsonResp.candidates[0].content.parts[0]
    ...
    var VALID_DATA_COLS = {
      'テキスト': true, 'ロングテキスト': true, '単一選択肢': true, '複数選択肢': true,
      '数値': true, '数値_小数点': true, 'パーセント': true, '電話番号': true,
      'メールアドレス': true, 'URL': true, '住所': true, '画像': true, 'ファイル': true,
      '日付': true, '日時': true
    };
    ...
    try {
      ...
      for (var key in aiResponseObject) {
        ...
        if (!dataType || !VALID_DATA_COLS[dataType]) { noMatch++; continue; }
        ...
        if (dataType === '単一選択肢') {
          ...
          if (sOpts && sOpts.length > 0) {
            ...
            writeValue = (value == null || String(value).trim() === '' || String(value) === '該当なし') ? '' : String(value);
          }
          ...
          if (mOpts && mOpts.length > 0) {
            ...
            var filteredValues = Array.isArray(mResult.values) ? mResult.values.filter(function(v) { return v !== '該当なし'; }) : [];
            writeValue = filteredValues.join('\n');
          } else {
            if (!value || (Array.isArray(value) && value.length === 0)) writeValue = '';
            else {
              var valArray = Array.isArray(value) ? value : [value];
              writeValue = valArray.filter(function(v) { return v != null && String(v) !== '該当なし'; }).join('\n');
            }
          }
        } else if (dataType === '数値' || dataType === '数値_小数点' || dataType === 'パーセント') {
          // 数値のクレンジング
          ...
          if (value == null || String(value).trim() === '' || String(value) === '該当なし') writeValue = '';
        ...
        // --- CloudSQL UPDATE ---
        var NON_TEXT_COLS = { '日付': true, '日時': true, '数値': true, '数値_小数点': true, 'パーセント': true };
        var updateSql;
        var isNonText = NON_TEXT_COLS[dataType];

        if (isNonText) {
          updateSql = 'UPDATE "帳票子レコード複製登録" SET "' + dataType + '" = ?, '
            + '"更新日時" = NOW() '
            + 'WHERE "帳票マスタ複製登録ID" = ? AND "&&項目名&&" = ? AND "項目データ型選択" = ?';
        } else {
          updateSql = 'UPDATE "帳票子レコード複製登録" SET "' + dataType + '" = '
            + 'CASE WHEN "' + dataType + '" IS NULL OR "' + dataType + '" = \'\' '
            + 'THEN ? '
            + 'ELSE "' + dataType + '" || E\'\\n\\n:\\n\' || ? '
            + 'END, '
            + '"更新日時" = NOW() '
      ...
      dbConn.commit();
      executionLog.push('CloudSQL書込完了: 更新 ' + writeCount + '件 (スキップ ' + skipCount + ', 対象外 ' + noMatch + ')');

    } catch (dbErr) {
      try { dbConn.rollback(); } catch (rbErr) { /* ignore */ }
      throw new Error('CloudSQL書込エラー: ' + dbErr.message);
    } finally {
      closeCloudSql_(dbConn);
    }

    var resultMessage = '✅ データ反映完了: 更新 ' + writeCount + '件 (空欄スキップ ' + skipCount + '件)';
...
function test_GeminiApiKeys() {
  ...
  var keyNames = ['API_KEY', 'API_KEY_2', 'API_KEY_3', 'API_KEY_4', 'API_KEY_5'];
  ...
  keyNames.forEach(function(keyName) {
    ...
    try {
      ...
      if (code === 200) {
        ...
        var answer = body.candidates && body.candidates[0] && body.candidates[0].content
          && body.candidates[0].content.parts && body.candidates[0].content.parts[0]
          && body.candidates[0].content.parts[0].text;
        Logger.log('✅ ' + keyName + ': 正常 (応答: ' + (answer || '').substring(0, 50) + ')');
      } else {
        Logger.log('❌ ' + keyName + ': エラー ' + code + ' - ' + response.getContentText().substring(0, 100));
      }
    } catch (e) {
