...
function STEP01_ChildAndMakeSpreadsheet_CloudSQL(
  ...
  try {
    const hasExisting =
      existingSpreadsheetUrl && String(existingSpreadsheetUrl).trim() !== "";
    ...
    // =====================================================
    const childResult = STEP01_Add_Childrescords_CloudSQL(
      parentid,
      refId,
      parentName,
    );
    if (typeof childResult === "string" && childResult.startsWith("❌"))
      return childResult;
...
function STEP01_Add_Childrescords_CloudSQL(parentid, refId, parentName) {
  ...
  try {
    ...
    const sheet_original =
      original.getSheetByName("ひな型帳票マスタ子レコード");
...
function recordPlaceholderPositionsToMaster_step01_(parentid, surceFileUrl) {
  ...
  if (!parentid)
    throw new Error("recordPlaceholderPositionsToMaster_: parentid が空です");
  if (!surceFileUrl)
    throw new Error(
      "recordPlaceholderPositionsToMaster_: surceFileUrl が空です",
    );

  const idMatch = String(surceFileUrl).match(/[-\w]{25,}/);
  if (!idMatch)
    throw new Error(
      "recordPlaceholderPositionsToMaster_: surceFileUrl からID抽出できません",
    );
  ...
  if (keyToPositions.size === 0) {
    console.log(
      "[recordPlaceholderPositionsToMaster_] &&...&& が見つかりませんでした",
    );
    return;
  }
  ...
  if (!masterSheet)
    throw new Error(
      `recordPlaceholderPositionsToMaster_: シート「${masterSheetName}」が見つかりません`,
    );
  ...
  if (isUpdated) {
    ...
    console.log(
      `[recordPlaceholderPositionsToMaster_] 一括記録完了: keys=${keyToPositions.size}`,
    );
  }
}
...
function copyExistingSpreadsheetIfNeeded_step01_(
  ...
  if (!existingSpreadsheetUrl || String(existingSpreadsheetUrl).trim() === "")
    return "";
  ...
  if (!targetFolder) {
    if (!customerDriveURL)
      return "エラー：保存先が特定できません（customerDriveURL が空）。";

    const customerFolder = DriveApp.getFolderById(
      String(customerDriveURL).match(/[-\w]{25,}/)[0],
    );

    const officeFolders = customerFolder.getFoldersByName(office);
    if (!officeFolders.hasNext())
      return `エラー：事業所フォルダ「${office}」が見つかりません。`;
    ...
    if (!targetFolder)
      return `エラー：フォルダ「${googleFolderName}」が見つかりません（待機タイムアウト）。`;

    if (zaisekiID) {
      try {
        postFolderUrlToExternalApp(targetFolder.getUrl(), zaisekiID);
      } catch (e) {}
    }
  }
