...
function startAsyncProcessFileAI_CloudSQL(
  ...
  try {
    ...
    var sql =
      'INSERT INTO "AIジョブキュー" ("ジョブID", "登録日時", "ジョブタイプ", "ペイロード", "状態") VALUES (?, NOW(), ?, ?, ?)';
...
function triggerDispatcherAsync_CloudSQL_() {
  try {
    ...
    Logger.log(
      "即時実行トリガーの作成に失敗（定時実行で処理されます）: " + e.message,
    );
  }
}
    ...
    tempTriggerIds.forEach(function (key) {
      ...
      if (now - timestamp > THRESHOLD) {
        if (triggerMap[triggerId]) {
          try {
            ScriptApp.deleteTrigger(triggerMap[triggerId]);
          } catch (e) {
            /* ignore */
          }
        }
...
function dispatcher_CloudSQL() {
  ...
  try {
    ...
    conn.commit();
    if (recovered > 0)
      Logger.log("リカバリ: " + recovered + "件をPendingに戻しました");
    ...
    while (Date.now() - start < 300000) {
      ...
      try {
        ...
        if (payload.ID) {
          try {
            updateAiResult_CloudSQL(
              payload.ID,
              "エラー: " + errMsg.substring(0, 200),
            );
          } catch (e2) {
            /* ignore */
          }
...
function insertShutsuryokusakiFile_CloudSQL(record) {
  ...
  try {
    ...
    if (record["記録対象期間：始"]) {
      stmt.setDate(
        9,
        Jdbc.newDate(new Date(record["記録対象期間：始"]).getTime()),
      );
    } else {
      stmt.setNull(9, 0);
    }
    if (record["記録対象期間：終"]) {
      stmt.setDate(
        10,
        Jdbc.newDate(new Date(record["記録対象期間：終"]).getTime()),
      );
    } else {
      stmt.setNull(10, 0);
    }
    ...
    if (record.登録日時) {
      stmt.setTimestamp(
        12,
        Jdbc.newTimestamp(new Date(record.登録日時).getTime()),
      );
    } else {
    ...
    if (record.更新日時) {
      stmt.setTimestamp(
        13,
        Jdbc.newTimestamp(new Date(record.更新日時).getTime()),
      );
    } else {
    ...
    if (record.AI帳票出力日時) {
      stmt.setTimestamp(
        16,
        Jdbc.newTimestamp(new Date(record.AI帳票出力日時).getTime()),
      );
    } else {
      stmt.setNull(16, 0);
    }
...
function updateAiResult_CloudSQL(fileId, resultMessage) {
  ...
  try {
    conn = getCloudSqlConnection_();
    var sql =
      'UPDATE "出力先ファイル" SET "AI帳票出力結果" = ?, "AI帳票出力日時" = ?, "更新日時" = ? WHERE "出力先ファイルID" = ?';
    stmt = conn.prepareStatement(sql);
...
function updateFlag_CloudSQL(fileId, flagValue) {
  ...
  try {
    conn = getCloudSqlConnection_();
    var sql =
      'UPDATE "出力先ファイル" SET "フラグ" = ?, "更新日時" = ? WHERE "出力先ファイルID" = ?';
    stmt = conn.prepareStatement(sql);
...
function test_shutsuryokusakiFile_CloudSQL() {
  ...
  try {
    ...
    closeCloudSql_(null, stmt, rs);
    stmt = conn.prepareStatement(
      'SELECT COUNT(*) as cnt FROM "出力先ファイル"',
    );
    rs = stmt.executeQuery();
...
function test_jobQueue_CloudSQL() {
  ...
  try {
    ...
    // 4. クリーンアップ
    stmt = conn.prepareStatement(
      'DELETE FROM "AIジョブキュー" WHERE "ジョブID" = ?',
    );
    stmt.setString(1, testJobId);
    stmt.executeUpdate();
