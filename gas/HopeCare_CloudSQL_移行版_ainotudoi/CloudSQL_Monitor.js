...
function monitorCloudSQL() {
  ...
  try {
    ...
    if (rs.next()) {
      ...
      if (total >= MONITOR_CONFIG.MAX_CONNECTIONS_CRITICAL) {
        alerts.push('🔴 *接続数 CRITICAL*: ' + total + '/200（active:' + active + ', idle:' + idle + ', idle_in_tx:' + idleInTx + '）');
      } else if (total >= MONITOR_CONFIG.MAX_CONNECTIONS_WARN) {
        alerts.push('🟡 *接続数 WARNING*: ' + total + '/200（active:' + active + ', idle:' + idle + ', idle_in_tx:' + idleInTx + '）');
      }
    }
    rs.close(); rs = null;
  ...
  if (alerts.length > 0) {
    ...
    var message = '📊 *CloudSQL 監視アラート*\n時刻: ' + timestamp + '\n\n' + alerts.join('\n\n');
    sendSlackNotification(message);
    Logger.log('監視アラート送信: ' + alerts.length + '件');
  } else {
    Logger.log('CloudSQL監視: 正常（' + Utilities.formatDate(new Date(), 'Asia/Tokyo', 'HH:mm:ss') + '）');
  }
}
...
function monitorSlowQueries() {
  ...
  try {
    ...
    if (!extensionEnabled) {
      Logger.log('pg_stat_statements 拡張が無効のため、pg_stat_activity から代替監視');
    ...
    while (rs.next()) {
      slowQueries.push(
        '  avg: ' + rs.getString('avg_ms') + 'ms' +
        ' / max: ' + rs.getString('max_ms') + 'ms' +
    ...
    if (slowQueries.length > 0) {
      var timestamp = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy/MM/dd HH:mm:ss');
      var message = '🐢 *スロークエリ検出*\n時刻: ' + timestamp +
        '\n閾値: 平均' + MONITOR_CONFIG.SLOW_QUERY_SECONDS + '秒超\n\n' +
        slowQueries.join('\n\n');
      sendSlackNotification(message);
    } else {
...
function monitorSlowQueriesFallback_(conn) {
  ...
  try {
    ...
    if (slowQueries.length > 0) {
      var timestamp = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy/MM/dd HH:mm:ss');
      var message = '🐢 *スロークエリ検出（実行中）*\n時刻: ' + timestamp +
        '\n閾値: ' + MONITOR_CONFIG.SLOW_QUERY_SECONDS + '秒超\n\n' +
        slowQueries.join('\n\n');
      sendSlackNotification(message);
    }
  } finally {
...
function sendDailyDBReport() {
  ...
  try {
    ...
    var tableCounts = [];
    while (rs.next()) {
      tableCounts.push('  ' + rs.getString('table_name') + ': ' + rs.getLong('row_count') + '行');
    }
    ...
    rs = stmt.executeQuery();
    if (rs.next()) {
      report.push('*接続数:* ' + rs.getInt('total') + ' / ' + rs.getInt('max_conn'));
    }

    var timestamp = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy/MM/dd HH:mm:ss');
    var message = '📋 *CloudSQL 日次レポート*\n時刻: ' + timestamp + '\nDB: hopecare\n\n' + report.join('\n\n');
    sendSlackNotification(message);
    ...
    var timestamp = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy/MM/dd HH:mm:ss');
    sendSlackNotification('🔴 *日次レポート失敗*\nエラー: ' + e.message + '\n時刻: ' + timestamp);
  } finally {
    closeCloudSql_(conn, stmt, rs);
  }
}
...
function setupMonitorTriggers() {
  ...
  // 10分おきヘルスチェック
  ScriptApp.newTrigger('monitorCloudSQL')
    .timeBased()
    .everyMinutes(10)
    .create();
  ...
  sendSlackNotification('✅ *CloudSQL監視を開始しました*\n• ヘルスチェック: 10分おき\n• スロークエリ監視: 15分おき\n• 日次レポート: 毎朝6時');
}
...
function removeMonitorTriggers() {
  ...
  var monitorFunctions = ['monitorCloudSQL', 'monitorSlowQueries', 'sendDailyDBReport'];
