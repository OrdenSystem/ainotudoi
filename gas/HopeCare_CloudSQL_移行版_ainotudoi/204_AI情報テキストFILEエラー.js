function checkErrorsAndNotifySlack() {
  try {
    ...
    if (!sheet) {
      throw new Error(`シート「${QUEUE_SHEET_NAME}」が見つかりません。名前が正しいか確認してください。`);
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (status === "ERROR" || status === "FAILED") {
        ...
        const messageText = `⚠️ *204_AI情報テキストFILEエラー検知*\n` +
    ...
    const systemErrorMessage = `🚨 *GAS実行システムエラー（204_AI情報テキストFILEトリガー失敗）*\n` +
...
function diagnoseAccessError() {
  ...
  if (TEST_SPREADSHEET_ID.includes("/") || TEST_SPREADSHEET_ID.includes("https")) {
    console.error("❌ 【診断】IDにURLが含まれています。URL全体ではなく、ID部分のみを抽出してください。");
    return;
  }
  if (TEST_SPREADSHEET_ID.includes(" ")) {
    console.error("❌ 【診断】IDに不要な空白が含まれています。前後のスペースを削除してください。");
    return;
  }

  // 2. 実際にアクセスを試みる
  try {
    console.log(`📄 ID: ${TEST_SPREADSHEET_ID} のファイルへアクセスを試みます...`);
    ...
    console.log("💡 現在のエラーは解消されています。もし本番コードでエラーが出る場合は、本番コード側のIDのコピペミスなどを確認してください。");
    ...
    if (e.message.includes("No item with the given ID could be found")) {
      ...
      console.log(`原因A: ID「${TEST_SPREADSHEET_ID}」が間違っている（削除された、または1文字欠けている等）。`);
      console.log(`原因B: 今GASを開いているGoogleアカウントに、このシートを開く権限がない。`);
      console.log("--------------------------------------------------");
      console.log("🛠️ 【解決策】");
      console.log("1. GASエディタの右上にあるプロフィールアイコンを見て、どのアカウントで作業しているか確認する。");
      console.log("2. 対象のスプレッドシートの「共有」設定から、そのアカウントに閲覧・編集権限を付与する。");
    } else {
      console.log("💡 予期せぬ別のエラーです。Googleのサーバー障害などの可能性があります。");
    }
  }
}