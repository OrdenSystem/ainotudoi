...
// ==========================================
const SEIKYU_TASK_SS_ID = getConfigId_("SEIKYU_TASK_SS_ID");
const SEIKYU_TASK_SHEET_NAME = "請求処理タスク";
...
function makeRecept(
  ...
  const ss = SpreadsheetApp.openById(SEIKYU_TASK_SS_ID);
  let sheet = ss.getSheetByName(SEIKYU_TASK_SHEET_NAME);
  if (!sheet) sheet = ss.insertSheet(SEIKYU_TASK_SHEET_NAME);
  initTaskSheet(sheet);
...
function makeReceptBackground() {
  ...
  const ss = SpreadsheetApp.openById(SEIKYU_TASK_SS_ID);
  const sheet = ss.getSheetByName(SEIKYU_TASK_SHEET_NAME);
  if (!sheet) return;
  ...
  if (!lock.tryLock(15000)) {
    console.warn(
      "⚠️システムの一時ロックが取得できませんでした。1分後に再試行します。",
    );
  ...
  try {
    ...
    if (isRunning) {
      console.log(
        "⏳ 現在、別のタスクが『実行中』です。順番待ちのため1分後に再チェックします。",
      );
      ScriptApp.newTrigger("makeReceptBackground")
        .timeBased()
        .after(60000)
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][1] === "待機中") {
        ...
        taskData = data[i];
        console.log(
          `💡対象タスク発見: 行番号 ${targetRowIndex}, 請求情報ID: ${taskData[4]}`,
        );
        break;
      }
    }
  ...
  try {
    ...
    result = _executeFn(
      ...
      p.TargetRows,
      p.ReSeikyu,
      p.TargetCustomerList,
      p.TargetNameList,
      p.TargetNameKanaList,
      p.TargetNumberList,
      p.TargetCityNameList,
      p.TargetKeyNameList,
      p.TargetKeyNameKanaList,
      p.TargetUpperPriceList,
    );

    if (result.url === "ERROR") {
      sendSlackNotification(
        `【レセプト生成エラー】請求情報ID: ${p.seikyuID}\n${result.log}`,
      );
    }
  } catch (e) {
    result = { url: "ERROR", log: "システムエラー: " + e.message };
    sendSlackNotification(
      `【レセプト生成システム致命的エラー】請求情報ID: ${p.seikyuID}\n詳細: ${e.message}`,
    );
  }
...
// ==========================================
/**
 * テスト: 請求AppのAppSheet API接続確認
 */
function test_AppSheetApi_Seikyu() {
  var props = PropertiesService.getScriptProperties();
  var APP_ID = props.getProperty("APPSHEET_APP_ID_請求app");
  var ACCESS_KEY = props.getProperty("APPSHEET_API_KEY_請求app");
  var TABLE_NAME = props.getProperty("APPSHEET_TABLE_NAME_請求app");

  Logger.log("APP_ID: " + (APP_ID ? APP_ID.substring(0, 8) + "..." : "未設定"));
  Logger.log(
    "ACCESS_KEY: " +
      (ACCESS_KEY ? ACCESS_KEY.substring(0, 8) + "..." : "未設定"),
  );
  Logger.log("TABLE_NAME: " + (TABLE_NAME || "未設定"));

  if (!APP_ID || !ACCESS_KEY || !TABLE_NAME) {
    Logger.log("❌ プロパティ未設定があります");
    return;
  }

  // Read（Find）で1件取得を試行
  var url =
    "https://www.appsheet.com/api/v2/apps/" +
    APP_ID +
    "/tables/" +
    encodeURIComponent(TABLE_NAME) +
    "/Find";
  var payload = {
    Selector: "Filter(請求情報DB, true)",
  ...
  if (code >= 200 && code < 300) {
    ...
    Logger.log("✅ Find成功。レコード数: " + rows.length);
    if (rows.length > 0) {
      var testId = rows[0]["請求情報ID"];
      Logger.log("先頭レコード 請求情報ID: " + testId);

      // Edit（Action）テスト - 処理結果に現在時刻を追記してみる
      Logger.log("--- Edit(Action)テスト ---");
      var editUrl =
        "https://www.appsheet.com/api/v2/apps/" +
        APP_ID +
        "/tables/" +
      ...
      var editPayload = {
        ...
        Rows: [
          {
            請求情報ID: testId,
            請求メモ: "APIテスト " + new Date().toLocaleTimeString("ja-JP"),
          },
        ],
      };
      var editOptions = {
      ...
      var editRes = UrlFetchApp.fetch(editUrl, editOptions);
      var editCode = editRes.getResponseCode();
      Logger.log("Edit ステータス: " + editCode);
      if (editCode >= 200 && editCode < 300) {
        Logger.log("✅ Edit成功");
      } else {
        Logger.log("❌ Edit失敗: " + editRes.getContentText());
      }
    }
  } else {
    Logger.log("❌ Find エラー: " + response.getContentText());
  }
}
...
function updateAppSheetRecord(seikyuID, fileUrl, logText) {
  ...
  if (!APP_ID || !ACCESS_KEY || !TABLE_NAME) {
    ...
    sendSlackNotification(
      `【AppSheet API設定エラー】請求情報ID: ${seikyuID}\n詳細: ${errMsg}`,
    );
    return;
  }
  ...
  try {
    ...
    if (responseCode >= 200 && responseCode < 300) {
      ...
      sendSlackNotification(
        `【AppSheet API書き戻し失敗】請求情報ID: ${seikyuID}\nStatus: ${responseCode}\n詳細: ${errMsg}`,
      );
    }
  } catch (e) {
    ...
    sendSlackNotification(
      `【AppSheet API通信致命的エラー】請求情報ID: ${seikyuID}\n詳細: ${e.message}`,
    );
  }
}
  ...
  try {
    ...
    const users = names.map((name, i) => {
      ...
      if (missingItems.length > 0) {
        unregisteredWarnings.push(
          `${name || "氏名不明"} (${missingItems.join(", ")})`,
        );
      }
    ...
    // ★ CloudSQL版: 01相談記録をスプシではなくCloudSQLから取得
    // ssDbData / ssDbHeader の配列形式を維持し、後続ロジックは一切変更なし
    const targetIds = Array.isArray(TargetRows)
      ? TargetRows.map(String)
      : String(TargetRows)
          .split(",")
          .map((s) => s.trim());

    var ssDbData, ssDbHeader;
    var _csqConn, _csqStmt, _csqRs;
    try {
      _csqConn = getCloudSqlConnection_();

      // ヘッダー取得（カラム名一覧）
      _csqStmt = _csqConn.prepareStatement(
        'SELECT * FROM "01相談記録" LIMIT 0',
      );
      _csqRs = _csqStmt.executeQuery();
      var _csqMeta = _csqRs.getMetaData();
      var _csqColCount = _csqMeta.getColumnCount();
      ssDbHeader = [];
      for (var _ci = 1; _ci <= _csqColCount; _ci++) {
        ssDbHeader.push(_csqMeta.getColumnName(_ci));
      }
      _csqRs.close();
      _csqStmt.close();

      // 対象IDのレコードを取得
      var _csqPlaceholders = targetIds
        .map(function () {
          return "?";
        })
        .join(", ");
      ...
      for (var _si = 0; _si < targetIds.length; _si++) {
        _csqStmt.setString(_si + 1, targetIds[_si]);
      }
      _csqRs = _csqStmt.executeQuery();
      ...
      while (_csqRs.next()) {
        var _csqRow = [];
        for (var _rj = 1; _rj <= _csqColCount; _rj++) {
          _csqRow.push(_csqRs.getString(_rj) || "");
        }
        _csqRows.push(_csqRow);
      }

      // ssDbData = [ヘッダー行, データ行...] の形式を維持
      ssDbData = [ssDbHeader].concat(_csqRows);
      log(
        "CloudSQLから01相談記録を取得: " +
          _csqRows.length +
          "件 (カラム数: " +
      ...
      closeCloudSql_(_csqConn, _csqStmt, _csqRs);
    }
    ...
    cityDbSheet
      ...
      .forEach((r) => {
        if (String(r[1]) === String(seikyuID))
          dbExistingCities.add(String(r[4]).trim());
      });
    ...
    const hIdx = {
      id: 0,
      city: ssDbHeader.indexOf("市町村"),
      cityNum: ssDbHeader.indexOf("市町村番号"),
      ym: ssDbHeader.indexOf("年月"),
      cat: ssDbHeader.indexOf("事業区分"),
      uid: ssDbHeader.indexOf("利用者在籍ID"),
      day: ssDbHeader.indexOf("日"),
      kihon: ssDbHeader.indexOf("基本報酬"),
      kasan: ssDbHeader.indexOf("加算"),
    ...
    ssDbData.slice(1).forEach((row) => {
      if (targetIds.indexOf(String(row[hIdx.id])) !== -1) {
        ...
        const ymCat =
          String(row[hIdx.ym]).replace(/年|月/g, "") + "_" + row[hIdx.cat];

        if (kihon !== "") countKihon++;
        if (kasan !== "") countKasan++;

        const cost1 =
          hIdx.cost1 !== -1 ? String(row[hIdx.cost1] || "").trim() : "";
        const cost2 =
          hIdx.cost2 !== -1 ? String(row[hIdx.cost2] || "").trim() : "";

        if (cName !== "" && cName !== "未登録") {
          if (!cityDataMap.has(cName))
            cityDataMap.set(cName, { name: cName, num: cNum });
        ...
        if (uID !== "") {
          ...
          if (targetUser) {
            ...
            if (kihon !== "") {
              let emptyIdx = targetUser.dailies.findIndex(
                (d) => d.day === day && d.kihon === "",
              );
              if (emptyIdx !== -1) {
                targetUser.dailies[emptyIdx].kihon = kihon;
                if (cost1 !== "" && targetUser.dailies[emptyIdx].cost1 === "")
                  targetUser.dailies[emptyIdx].cost1 = cost1;
                if (cost2 !== "" && targetUser.dailies[emptyIdx].cost2 === "")
                  targetUser.dailies[emptyIdx].cost2 = cost2;
              } else {
                if (
                  !targetUser.dailies.some(
                    (d) => d.day === day && d.kihon === kihon,
                  )
                ) {
                  targetUser.dailies.push({
                    day: day,
                    kihon: kihon,
                    cost1: cost1,
                    cost2: cost2,
              ...
              if (!existing) {
                ...
                if (cost1 !== "" && existing.cost1 === "")
                  existing.cost1 = cost1;
                if (cost2 !== "" && existing.cost2 === "")
                  existing.cost2 = cost2;
              }
            }
    ...
    if (cityWriteRows.length > 0)
      cityDbSheet
        .getRange(
          cityDbSheet.getLastRow() + 1,
          1,
          cityWriteRows.length,
          cityWriteRows[0].length,
        )
        .setValues(cityWriteRows);
    if (userWriteRows.length > 0)
      userBaseDbSheet
        .getRange(
          userBaseDbSheet.getLastRow() + 1,
          1,
          userWriteRows.length,
          userWriteRows[0].length,
        )
        .setValues(userWriteRows);
    ...
    newSS.getSheets().forEach((s) => {
      if (s.getName() !== category && newSS.getSheets().length > 1)
        newSS.deleteSheet(s);
    });
    ...
    if (kasanDbSheet) {
      ...
      kData
        .filter((r) => String(r[1]) === String(seikyuID))
        .forEach((kr) => {
          const rIdx = allValues.findIndex(
            (r) => String(r[0]).trim() === String(kr[4]).trim(),
          );
    ...
    let cityRowIdx =
      currentA.findIndex((r) => String(r[0]).trim() === "市町村") + 1;
    ...
    if (cityRowIdx > 0 && cityDataList.length > 0) {
      ...
      if (cityDataList.length > 1) {
        ...
        targetSheet.insertRowsAfter(
          insertPosition,
          (cityDataList.length - 1) * cityBlockSize,
        );
        const source = targetSheet.getRange(
          cityRowIdx,
          1,
          cityBlockSize,
        ...
        for (let i = 1; i < cityDataList.length; i++) {
          source.copyTo(
            targetSheet.getRange(cityRowIdx + i * cityBlockSize, 1),
          );
        }
      }
      ...
      const cleanRange = targetSheet.getRange(
        cityRowIdx,
        1,
        targetSheet.getLastRow() - cityRowIdx + 1,
        2,
      );
    ...
    if (users.length > 0) {
      ...
      const uBase =
        currentA.findIndex((r) => String(r[0]).trim() === "氏名") + 1;
      ...
      if (uBase > 0 && dRow > 0) {
        ...
        for (let i = 0; i < users.length; i++) {
          ...
          const scanRange = targetSheet.getRange(
            currentStartRow,
            1,
            blockSize + 10,
            1,
          ...
          if (limitRowsNeeded > 0 && limitHeaderRelIdx !== -1) {
            ...
            targetSheet
              .getRange(insertPos, 1, 1, targetSheet.getLastColumn())
              .copyTo(
          ...
          for (let r = 0; r < scanVals.length; r++) {
            ...
            if (category === "障害児相談支援") {
              if (txt === "氏名")
                targetSheet.getRange(rowIdx, 2).setValue(user.kName);
              else if (txt === "氏名カナ")
                targetSheet.getRange(rowIdx, 2).setValue(user.kKana);
              else if (txt === "児童氏名")
                targetSheet.getRange(rowIdx, 2).setValue(user.name);
              else if (txt === "児童氏名カナ")
                targetSheet.getRange(rowIdx, 2).setValue(user.kana);
            } else {
              if (txt === "氏名")
                targetSheet.getRange(rowIdx, 2).setValue(user.name);
              else if (txt === "氏名カナ")
            ...
            if (txt === "受給者証番号")
              targetSheet.getRange(rowIdx, 2).setValue(user.num);
            else if (txt === "支給市町村")
              targetSheet.getRange(rowIdx, 2).setValue(user.city);
            else if (txt === "利用者負担上限額")
              targetSheet.getRange(rowIdx, 2).setValue(user.up);
    ...
    if (unregisteredWarnings.length > 0) {
      log(
        "【確認事項】以下の利用者に「未登録」のデータが含まれています。スプレッドシートを確認してください：",
      );
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (cellA === "日" && cellC === "種類") {
      ...
      if (rowsInBlock.length > 0) {
        const blockSummary = rowsInBlock
          .map((x) => `[行${x.sheetRow}:日${x.day}/種${x.type}]`)
          .join(", ");
      ...
      const mergePair = (typeA, typeB, mergedType) => {
        ...
        if (itemA && itemB) {
          logger(
            `  ★統合対象発見: ${typeA}(行${itemA.sheetRow}) & ${typeB}(行${itemB.sheetRow}) -> ${mergedType}`,
          );
...
function applyFinalReplacements(targetSpreadsheetId, configSpreadsheetId) {
  ...
  const configValues = configSheet
    .getRange(2, 1, lastConfigRow - 1, 8)
    .getValues();
  ...
  configValues.forEach((row) => {
    ...
    foundCells.forEach((cell) => {
      try {
        ...
        else if (locationRule === "1つ右のセル")
          cell.offset(0, 1).setValue(replaceValue);
        else if (locationRule === "1つ下のセル")
          cell.offset(1, 0).setValue(replaceValue);
      } catch (e) {
