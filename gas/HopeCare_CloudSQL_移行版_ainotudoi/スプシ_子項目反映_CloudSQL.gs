...
function processPendingTasks_ssToChild_CloudSQL() {
  ...
  try {
    ...
    const logResult = executeHeavyTask_ssToChild_CloudSQL_(
      spreadsheetUrl,
      reportMasterId,
    );
...
function executeHeavyTask_ssToChild_CloudSQL_(spreadsheetUrl, reportMasterId) {
  ...
  try {
    ...
    if (values.length < 2)
      return "【終了】対象の子レコードがデータベースに0件でした。";
    ...
    for (let r = 1; r < values.length; r++) {
      ...
      for (const pos of posList) {
        ...
        const rawValue = getCachedCellValue_ssToChild_(
          reportSS,
          parsed.sheetName,
          parsed.a1,
          sheetCaches,
        ...
        const cleaned = stripAmpPlaceholders_ssToChild_(
          String(rawValue ?? ""),
        ).trim();
        if (cleaned !== "") {
          pickedValue = cleaned;
    ...
    if (updateList.length > 0) {
      ...
      for (const u of updateList) {
        try {
          var stmt = updConn.prepareStatement(
    ...
    logs.push(
      `・不成立：空欄等${空値} / シート名不一致${シート無} / セル位置不正${位置不正}`,
    );
    if (DB反映エラー > 0)
      logs.push(`・DB反映エラー：${DB反映エラー} 件（型不一致など）`);
...
function openDb_ssToChild_CloudSQL_(reportMasterId) {
  ...
  var stmt = conn.prepareStatement(
    'SELECT * FROM "帳票子レコード複製登録" WHERE "帳票マスタ複製登録ID" = ?',
  );
  ...
  conn.close();
  return {
    idx: result.headers.reduce((a, c, i) => {
      a[c] = i;
      return a;
    }, {}),
    allData: [result.headers].concat(result.rows),
  };
}
...
function getCachedCellValue_ssToChild_(ss, sheetName, a1, caches) {
  if (!caches[sheetName]) {
    ...
    caches[sheetName] =
      maxRow === 0 || maxCol === 0
        ? []
        : sheet.getRange(1, 1, maxRow, maxCol).getDisplayValues();
  }
