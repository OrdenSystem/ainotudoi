...
function test_AppSheetApi_Seikyu() {
  ...
  Logger.log("ACCESS_KEY: " + (ACCESS_KEY ? ACCESS_KEY.substring(0, 8) + "..." : "未設定"));
  ...
  // Read（Find）で1件取得を試行
  var url = "https://www.appsheet.com/api/v2/apps/" + APP_ID + "/tables/" + encodeURIComponent(TABLE_NAME) + "/Find";
  var payload = {
  ...
  if (code >= 200 && code < 300) {
    ...
    if (rows.length > 0) {
      ...
      var editUrl = "https://www.appsheet.com/api/v2/apps/" + APP_ID + "/tables/" + encodeURIComponent(TABLE_NAME) + "/Action";
      var editPayload = {
        "Action": "Edit",
...
function executeMakeRecept( 
  ...
  try {
    ...
    try {
      ...
      // ヘッダー取得（カラム名一覧）
      _csqStmt = _csqConn.prepareStatement('SELECT * FROM "01相談記録" LIMIT 0');
      ...
      // 対象IDのレコードを取得
      var _csqPlaceholders = targetIds.map(function() { return '?'; }).join(', ');
      _csqStmt = _csqConn.prepareStatement(
        'SELECT * FROM "01相談記録" WHERE "相談記録ID" IN (' + _csqPlaceholders + ')'
      );
      ...
      log("CloudSQLから01相談記録を取得: " + _csqRows.length + "件 (カラム数: " + _csqColCount + ")");
...
function generateRandomId(l){const c='abcdefghijklmnopqrstuvwxyz0123456789';let r='';for(let i=0;i<l;i++)r+=c.charAt(Math.floor(Math.random()*c.length));return r}
