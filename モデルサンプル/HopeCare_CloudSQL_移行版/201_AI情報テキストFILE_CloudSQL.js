...
// ✨ 引数に sheet と rowIndex を追加 (キュー処理からの呼び出し用)
function generateAIContextFile(targetId, options, sheet, rowIndex) {
  ...
  try {
    ...
    // 2) 利用者基本情報
    // -------------------------------------------------------
    if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ SQL接続準備中... [基本情報取得]");
    ...
    if (!isFirstRun) {
      if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ 既存履歴ファイル読み込み中...");
    ...
    var COLS_SOUDAN = ['相談記録ID', '更新日時', '記録日', '相談種別', 'タイトル', '基本報酬', '加算', '連携先の機関'];
    var COLS_CASE = ['Row ID', '更新日時', '記録者', '支援記録種別', '記録全容'];
    var COLS_FORM = ['帳票マスタ複製登録ID', '更新日時', '帳票名', '帳票作成処理結果', 'スプシURL'];
    var COLS_FORM_CHILD = null;
    ...
    try {
      ...
      // 5-1) 01相談記録
      if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ SQL: 01相談記録を抽出中... [1/4]");
      
      const rowsConsult = isFirstRun
        ? selectAsObjects_(_ctxConn, '01相談記録', '"利用者在籍ID" = ?', [userId], COLS_SOUDAN)
        : selectAsObjects_(_ctxConn, '01相談記録', '"利用者在籍ID" = ? AND "更新日時" >= ? AND "更新日時" < ?', [userId, start, end], COLS_SOUDAN);
      ...
      // 5-2) ケース記録
      if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ SQL: ケース記録を抽出中... [2/4]");
      
      const rowsCase = isFirstRun
        ? selectAsObjects_(_ctxConn, 'ケース記録', '"利用者在籍ID" = ?', [userId], COLS_CASE)
        : selectAsObjects_(_ctxConn, 'ケース記録', '"利用者在籍ID" = ? AND "更新日時" >= ? AND "更新日時" < ?', [userId, start, end], COLS_CASE);
      ...
      // 5-3) 帳票マスタ
      if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ SQL: 帳票マスタを抽出中... [3/4]");
      
      const rowsForms = isFirstRun
        ? selectAsObjects_(_ctxConn, '帳票マスタ複製登録', '"利用者在籍ID" = ?', [userId], COLS_FORM)
        : selectAsObjects_(_ctxConn, '帳票マスタ複製登録', '"利用者在籍ID" = ? AND "更新日時" >= ? AND "更新日時" < ?', [userId, start, end], COLS_FORM);
      ...
      // 5-4) 帳票子レコード
      if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ SQL: 帳票子レコードを抽出中... [4/4]");
      ...
      if (isFirstRun) {
        ...
        if (pIds.length > 0) {
          ...
          rowsFormDetails = selectAsObjects_(_ctxConn, '帳票子レコード複製登録', '"帳票マスタ複製登録ID" IN (' + ph + ')', pIds, COLS_FORM_CHILD);
        }
      } else {
        // 当日更新分
        const todayChild = selectAsObjects_(_ctxConn, '帳票子レコード複製登録', '"更新日時" >= ? AND "更新日時" < ?', [start, end], COLS_FORM_CHILD);
        ...
        if (pIdsToday.length > 0) {
          ...
          const parentChild = selectAsObjects_(_ctxConn, '帳票子レコード複製登録', '"帳票マスタ複製登録ID" IN (' + ph2 + ')', pIdsToday, COLS_FORM_CHILD);
    ...
    // 5-5) 音声記録
    if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "⏳ AppSheet: 音声記録を抽出中...");
    ...
    if(sheet && rowIndex) updateQueueMemo_(sheet, rowIndex, "💾 Google Driveにテキストファイルを保存中...");
    Logger.log("💾 保存中...");
...
//
// columns (任意, 第5引数):
//   - 配列で必要カラム名を指定すると SELECT "c1","c2",... の形になり、
//     getString 呼出回数が劇的に減って高速化（ケース記録 82 列 → 5 列で 16 倍速）
//   - 未指定 / null / 空配列 → 従来通り SELECT * （後方互換）
// ==================================================================================
/**
 * 【診断用】指定テーブルの実カラム名を CloudSQL から取得してログ出力する。
 * SELECT * LIMIT 0 で MetaData だけ取り出すので軽量。
 * GAS UI で diagnose_dumpTableColumns_('帳票子レコード複製登録') を実行して使う。
 */
function diagnose_dumpTableColumns_(tableName) {
  var conn, stmt, rs;
  try {
    conn = getCloudSqlConnection_();
    stmt = conn.prepareStatement('SELECT * FROM "' + tableName + '" LIMIT 0');
    rs = stmt.executeQuery();
    var meta = rs.getMetaData();
    var n = meta.getColumnCount();
    Logger.log('===== ' + tableName + ' columns (' + n + ') =====');
    for (var i = 1; i <= n; i++) {
      Logger.log('  ' + i + ': ' + meta.getColumnName(i) + ' (' + meta.getColumnTypeName(i) + ')');
    }
  } catch (e) {
    Logger.log('🚨 diagnose_dumpTableColumns_ エラー: ' + e.message);
  } finally {
    closeCloudSql_(conn, stmt, rs);
  }
}

function selectAsObjects_(conn, tableName, whereClause, params, columns) {
  var stmt, rs;
  try {
    var selectClause = '*';
    if (columns && columns.length > 0) {
      ...
      for (var k = 0; k < columns.length; k++) {
        quoted.push('"' + columns[k] + '"');
      }
      selectClause = quoted.join(', ');
    }
