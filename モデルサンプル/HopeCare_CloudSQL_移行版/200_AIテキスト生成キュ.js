...
function enqueueAIContextTask(targetId) {
  ...
  // ★追加: 同時書き込みの衝突を防ぐ順番待ちロック（最大10秒待機）
  ...
  try {
    ...
    Logger.log(`⚠️ キュー登録エラー: ${e.message}`);
    return { result: "Error", error: e.message };
  } finally {
    // ★追加: 書き込みが終わったら必ずロックを解除して次の人を案内する
    lock.releaseLock();
  }
}
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (status === "PENDING" || status === "RETRY" || status === "RESET_PENDING") {
        ...
        try {
          ...
          // ✨ 追加: 処理開始のメッセージを書き込み
          updateQueueMemo_(sheet, rowIndex, "⏳ 処理開始: AIテキスト生成準備中...");
          ...
          // ✨ 変更: generateAIContextFileに、sheetとrowIndexを引き渡す
          generateAIContextFile(targetId, { isFirstRun: isReset, cachedStaffList: cachedStaffList }, sheet, rowIndex);
...
// ===============================================
// ✨ 新規追加: シートの進捗メッセージをリアルタイム更新するヘルパー関数
// ===============================================
function updateQueueMemo_(sheet, rowIndex, message) {
  try {
    sheet.getRange(rowIndex, 5).setValue(message); // 5列目(memo)に書き込み
    SpreadsheetApp.flush(); // 即時反映
  } catch (e) {
    Logger.log("⚠️ メモ更新失敗: " + e.message);
  }
}