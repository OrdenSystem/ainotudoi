...
function callAppSheetApi(appId, apiKey, tableName, selector) {
  ...
  const options = {
    ...
    payload: JSON.stringify({ 
      Action: "Find", 
      Properties: { Locale: "ja-JP", Timezone: "Tokyo Standard Time", Selector: selector }, 
      Rows: [] 
    }),
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (code === 200) {
      ...
      if (attempt === MAX_RETRIES) {
        throw new Error(`API Error (${tableName}): 429エラーが続き、リトライ上限に達しました。`);
      }
      
      // 待機時間を徐々に長くする (3秒 → 6秒 → 12秒 → 24秒 → 48秒)
      const waitTime = Math.pow(2, attempt) * 3000;
      Logger.log(`⚠️ AppSheet API制限(429): ${tableName} への通信を ${waitTime/1000}秒待機して再試行します (${attempt + 1}/${MAX_RETRIES})`);
      Utilities.sleep(waitTime);
      
    } else {
      // 429以外のエラー (400や500など、待って解決しない致命的エラー)
      throw new Error(`API Error (${tableName}): HTTP ${code} - ${res.getContentText()}`);
    }
  }
}