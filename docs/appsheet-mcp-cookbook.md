...
## 付録 0: Selector の書式（find_records 用）

**Issue #7 由来の重要事項**。

`appsheet_find_records` の `selector` 引数は **AppSheet API v2 のリスト関数形式**が必須。boolean 式単体（例: `[列] = "値"`）を渡すとエラーが出ず **0 件返却**になり、デバッグが辛い。

### 推奨書式

```
✅ Filter("テーブル名", boolean式)
   例: Filter("記事管理", [ステータス] = "未処理")
   例: Filter("ログ", AND([記録日時] >= TODAY(), [区分] = "食事"))

✅ Select(テーブル[列], 条件)
   例: Select(記事管理[ID], [ステータス] = "公開済み")
...
❌ TRUE                                      ← 0 件返却
❌ [ステータス] = "未処理"                   ← 0 件返却
❌ AND([記録日時] >= TODAY(), ...)           ← 0 件返却
❌ DATE([記録日時]) = TODAY()                ← 0 件返却
```

### Phase 6 以降の自動ラップ

`appsheet_find_records` ツールは boolean 式単体を受け取った場合、**自動的に `Filter("テーブル名", ...)` でラップ**する。既に `Filter(...)` `Select(...)` `TopN(...)` 等のリスト関数で始まる selector は素通し（後方互換）。

### 鉄板パターン

**今日の記録だけ取得**:
```
selector: 'Filter("ログ", AND([記録日時] >= TODAY(), [記録日時] < TODAY() + 1))'
```

**自分の担当案件**:
```
selector: 'Filter("案件", [担当者メール] = USEREMAIL())'
```

**Bot トリガー条件相当**:
```
selector: 'Filter("帳票", AND([完了フラグ], ISBLANK([展開UserMail])))'
```
